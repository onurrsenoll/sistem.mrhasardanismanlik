...
MR.CrmAramaPage = ({setPage, user}) => {
  ...
  /* ═══ YETKİ KONTROLÜ ═══
     Admin kullanıcı bypass edilir. Diğer kullanıcılarda crm/crm-arama izni zorunlu.
     Yetkisi yoksa "Yetkisiz Erişim" ekranı gösterilir, veri yüklenmez. */
  if (MR.yetki && !MR.yetki.kontrol(user, 'crm', 'crm-arama')) {
    return React.createElement(MR.yetki.YetkisizEkran, {
      mesaj: 'ARAMA LİSTESİNİ GÖRÜNTÜLEME YETKİNİZ BULUNMAMAKTADIR',
      ayrinti: 'Yöneticinizden yetki matrisi üzerinden "CRM / SAHA > ARAMA LİSTESİ GÖRÜNTÜLE" iznini talep edin.'
    });
  }

  /* Buton/aksiyon düzeyinde yetki bayrakları (frontend'te gizleme; backend de ayrıca kontrol eder) */
  const yetkiEkle       = !MR.yetki || MR.yetki.kontrol(user, 'crm', 'crm-arama-ekle');
  const yetkiDuzenle    = !MR.yetki || MR.yetki.kontrol(user, 'crm', 'crm-arama-duzenle');
  const yetkiSil        = !MR.yetki || MR.yetki.kontrol(user, 'crm', 'crm-arama-sil');
  const yetkiExcel      = !MR.yetki || MR.yetki.kontrol(user, 'crm', 'crm-arama-excel');
  const yetkiTopluIslem = !MR.yetki || MR.yetki.kontrol(user, 'crm', 'crm-arama-toplu-islem');
