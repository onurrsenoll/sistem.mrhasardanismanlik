...
MR.webrtcTelefon = {
  ...
  /* ═══ BAŞLAT ═══
     Singleton: Aynı UA'yı ikinci kez başlatmaz (`_ua` set ise sessizce çık).
     Oturum boyunca tek bağlantı ayakta kalır; sayfa geçişi UA'yı koparmaz. */
  baslat: function(config) {
    if (this._ua) {
      console.log('[WEBRTC] UA zaten başlatılmış — singleton koruması, yeniden bağlanma yok.');
      return;
    }
    ...
    /* beforeunload — Sayfa/sekme kapatıldığında UA'yı düzgün sonlandırıp PBX kayıt bırakma. */
    if (!this._beforeUnloadKurulu) {
      var selfUnload = this;
      window.addEventListener('beforeunload', function() {
        try { if (selfUnload._ua) selfUnload._ua.stop(); } catch(e) {}
      });
      this._beforeUnloadKurulu = true;
    }
