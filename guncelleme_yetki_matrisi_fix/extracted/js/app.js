...
const MENU = [
  ...
  {id:'crm', label:'CRM / SAHA', icon:'Users', sub:[
    /* 'crm-liste' (CRM LİSTESİ) kaldırıldı — 'crm-arama' (ARAMA LİSTESİ) aynı görevi üstlenir. */
...
const Breadcrumb = ({page, setPage}) => {
  ...
  for (const m of MENU) {
    ...
    if (page.startsWith('saha-')) {
      if (m.id === 'crm') {
        parts.push({label: 'CRM / SAHA', id: 'saha-liste'});
...
const App = () => {
  ...
  /* ═══ WEBRTC / NETSANTRAL OTOMATİK BAĞLANTI (SINGLETON)
     Kullanıcı sisteme girince (ayarlar tablosunda NetSantral tanımı varsa) WebRTC başlar
     ve oturum boyunca bağlı kalır. Sayfa geçişinde ya da re-render'da KOPMAZ
     (MR.webrtcTelefon tek global, baslat() singleton). */
  useEffect(() => {
    if (!user) return;
    if (!MR.webrtcTelefon) return;
    /* Yetki: cagri-yap yoksa bile widget yetkisi varsa bağlansın ki gelen çağrıları görebilsin. */
    const yetkiCagriYap = MR.yetki && MR.yetki.kontrol(user, 'cagri_merkezi', 'cagri-yap');
    const yetkiWidget  = MR.yetki && MR.yetki.kontrol(user, 'cagri_merkezi', 'widget-goruntule');
    if (!yetkiCagriYap && !yetkiWidget) {
      console.log('[WEBRTC-AUTO] Kullanıcının çağrı merkezi yetkisi yok — bağlantı açılmıyor.');
      return;
    }
    (async () => {
      try {
        ...
        const a = r.data;
        const cfg = {
          wssUrl:    a.webrtc_wss_url    || a.netsantral_wss_url    || '',
          domain:    a.webrtc_domain     || a.netsantral_domain     || '',
          dahili:    a.webrtc_dahili     || a.netsantral_dahili     || '',
          sipSifre:  a.webrtc_sip_sifre  || a.netsantral_sip_sifre  || '',
          kullanici: a.webrtc_kullanici  || a.netsantral_kullanici  || '',
          apiSifre:  a.webrtc_api_sifre  || a.netsantral_api_sifre  || '',
          santralNo: a.webrtc_santral_no || a.netsantral_santral_no || '',
          turnUrl:   a.webrtc_turn_url   || '',
          turnUser:  a.webrtc_turn_user  || '',
          turnPass:  a.webrtc_turn_pass  || ''
        };
        if (!cfg.dahili || !cfg.sipSifre || !cfg.wssUrl) {
          console.log('[WEBRTC-AUTO] NetSantral ayarları eksik, bağlantı açılmıyor.');
          return;
        }
        /* Tek sefer başlat — ikinci çağrı içinde singleton koruma var. */
        MR.webrtcTelefon.baslat(cfg);
      } catch(e) {
        console.warn('[WEBRTC-AUTO] Ayarlar yüklenemedi:', e);
      }
    })();
    /* Bu useEffect'in cleanup'ı yok: oturum boyunca bağlantı devam etsin. Logout'ta durdur() çağrılır. */
  }, [user?.id]);
  ...
  const logout = () => {
    /* Oturumdan çıkarken WebRTC bağlantısını kapat — PBX'de hayalet kayıt kalmasın. */
    try { if (MR.webrtcTelefon && typeof MR.webrtcTelefon.durdur === 'function') MR.webrtcTelefon.durdur(); } catch(e) {}
    api.setToken(null);
    setUser(null);
    setPage('home');
  };
  ...
  return (
    <div style={{minHeight: '100vh', background: C.bg, color: C.text, position: 'relative', overflow: 'hidden'}}>
      ...
      {/* GELEN ÇAĞRI WİDGET — Sadece 'cagri_merkezi / widget-goruntule' yetkisi olanlarda gösterilir.
          Widget'in içinde de yetki kontrolü var (listener register olurken MR.yetki.kontrol çalışır). */}
      {MR.GelenCagriWidget ? <MR.GelenCagriWidget /> : null}
