...
/* ─── ORTAK STİLLER ─── */
const TH_STYLE = {padding:'12px 14px', fontSize:13, fontWeight:800, color:'#fff', textAlign:'left', borderBottom:`2px solid ${MR.C.border}`, letterSpacing:0.5, background: MR.tema==='koyu'?'#1a3f8a':'#1a56db'};
              ...
              <table style={{width:'100%', borderCollapse:'collapse', minWidth:700}}>
                ...
                <tbody>
                  {filtrelenmis.map((d, idx) => (
                    <tr key={d.id} style={{transition:'all .2s', opacity: d.aktif == 0 ? 0.55 : 1, backgroundColor:MR.tema==='koyu'?(idx%2===0?'#1e3a78':'#172e5e'):(idx%2===0?'#ffffff':'#f4f7ff'), backgroundImage:MR.tema==='koyu'?(idx%2===0?'linear-gradient(90deg, #1e3a78 0%, #172e5e 100%)':'linear-gradient(90deg, #172e5e 0%, #0f2347 100%)'):'none', borderBottom:MR.tema==='koyu'?'1px solid rgba(59,130,246,0.25)':'1px solid #e8edf8', borderLeft:MR.tema==='koyu'?'3px solid rgba(59,130,246,0.5)':'3px solid rgba(26,86,219,0.35)', boxShadow:MR.tema==='koyu'?'0 2px 6px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.04)':'0 1px 3px rgba(26,86,219,0.06)', borderRadius:6}}
                      ...
                      onMouseLeave={e => {e.currentTarget.style.backgroundColor=MR.tema==='koyu'?(idx%2===0?'#1e3a78':'#172e5e'):(idx%2===0?'#ffffff':'#f4f7ff');e.currentTarget.style.backgroundImage=MR.tema==='koyu'?(idx%2===0?'linear-gradient(90deg, #1e3a78 0%, #172e5e 100%)':'linear-gradient(90deg, #172e5e 0%, #0f2347 100%)'):'none';e.currentTarget.style.boxShadow=MR.tema==='koyu'?'0 2px 6px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.04)':'0 1px 3px rgba(26,86,219,0.06)';e.currentTarget.style.transform='translateY(0)';}}>
