...
MR.SoftphoneWidget = ({
  ...
  const tusTakimi = [['1', '2', '3'], ['4', '5', '6'], ['7', '8', '9'], ['*', '0', '#']];
  ...
  const ara = async () => {
    ...
    const r = await api.netsantralCagriBaslat({
      numara
    });
    if (r?.success) {
      setDurum('gorusme');
  ...
  }, 'NETSANTRAL')), React.createElement('div', {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
  ...
  }, durumText[durum]), React.createElement('div', {
  ...
  React.createElement('div', {
    ...
    value: numara,
    onChange: e => setNumara(e.target.value.replace(/[^0-9*#+ ]/g, '')),
    placeholder: 'Numara girin',
    onKeyDown: e => {
      if (e.key === 'Enter') ara();
    }
  }), numara && React.createElement('div', {
    onClick: sil,
    style: {
      cursor: 'pointer',
      padding: 4
    ...
    size: 16,
    color: C.textMuted
  }))), durumMesaj && React.createElement('div', {
    style: {
      fontSize: 10,
      color: durumRenk[durum],
      fontWeight: 600,
  ...
  }, durumMesaj)), /* TUŞ TAKIMI */
  React.createElement('div', {
    style: {
      padding: '8px 16px 12px'
    }
  }, tusTakimi.map((satir, si) => React.createElement('div', {
    key: si,
    style: {
      display: 'flex',
      gap: 8,
  ...
  }, satir.map(t => React.createElement('div', {
    key: t,
    onClick: () => tusEkle(t),
    style: {
      width: 60,
  ...
  React.createElement('div', {
    ...
    onClick: ara,
    disabled: durum !== 'hazir' || !numara || numara.length < 4,
    style: {
      ...S.btn,
      ...S.btnS,
      flex: 1,
    ...
    size: 16,
    color: '#fff'
  }), 'ARA'), numara && React.createElement('button', {
    onClick: temizle,
    style: {
      ...S.btn,
      ...S.btnG,
  ...
  React.createElement('div', {
    ...
    style: {
      ...
      justifyContent: 'center',
      cursor: 'pointer',
      boxShadow: `0 8px 24px ${acik ? C.danger : C.success}44`,
      transition: 'all .3s',
      transform: acik ? 'rotate(135deg)' : 'rotate(0deg)'
    },
    onMouseEnter: e => {
      e.currentTarget.style.transform = acik ? 'rotate(135deg) scale(1.1)' : 'scale(1.1)';
    },
    onMouseLeave: e => {
      e.currentTarget.style.transform = acik ? 'rotate(135deg)' : 'rotate(0deg)';
    }
  }, React.createElement(LIcon, {
    name: acik ? 'X' : 'Phone',
    size: 24,
