...
try {
    ...
    $stmt->execute([
        $dosyaNo,
        $dosyaTuru,
        clean($kayit['sigorta_sirketi'] ?? ''),
        clean($kayit['police_no'] ?? ''),
        $kayit['hasar_tarihi'],
        clean($kayit['plaka'] ?? ''),
    ...
    // 6. Saha dosyasını güncelle
    $stmt = $db->prepare("UPDATE saha_dosyalar SET durum = 'dosyaya_donustu', dosya_id = ?, dosyaya_donusme_tarihi = NOW() WHERE id = ?");
    $stmt->execute([$dosyaId, $id]);

    $db->commit();

    // 7. Personele bildirim gönder
