...
function menuErisim(user) {
  ...
  /* YETKİLER VARSA → SADECE VERİTABANINDAKİ İZİNLERE GÖRE FİLTRELE
     v2.9.2: Yetki anahtar adlandırması farklılıklarına karşı esnek kontrol —
     bir modülün HERHANGİ bir izni varsa ana menü item'ı görünür.
     Önceki sürümde sadece '<modul>_goruntule' veya '<modul>_<modul>-goruntule'
     pattern'i kabul ediliyordu, ama yetki listesinde 'mesaj-goruntule',
     'bildirim-goruntule' gibi farklı isimli anahtarlar olabiliyor —
     onlar yakalanamıyordu. */
  return MENU.map(m => {
    ...
    /* SUB YOK (AJANDA, E-POSTA, BİLDİRİMLER GİBİ): modülün herhangi bir
       izni varsa göster — 'mesaj-goruntule', 'bildirim-goruntule',
       'eposta-goruntule', 'ajanda-goruntule' vb. tümü kabul edilir. */
    const prefix = modul + '_';
    const herhangiYetkiVar = Object.keys(yetkiler).some(k =>
      k.indexOf(prefix) === 0 && yetkiler[k] === 1
    );
    return herhangiYetkiVar ? m : null;
  }).filter(Boolean);
}
...
/* ═══ v2.1: İZOLE BİLDİRİM BADGE — TopNav'ı re-render etmez (focus-loss fix) ═══ */
const BildirimBadge = React.memo(function BildirimBadge() {
  ...
  return (
    <span style={{
      ...
      background: MR.C.danger, color: '#fff', fontSize: 8,
      display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700
    }}>{count > 9 ? '9+' : count}</span>
  );
});

/* ═══ v2.1: İZOLE MAIL BADGE — TopNav'ı re-render etmez (focus-loss fix) ═══ */
const MailBadge = React.memo(function MailBadge({user, isK}) {
  const [count, setCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const r = await MR.api.mailYeniSayisi();
        if (!alive || !r?.success) return;
        const yeniSayi = (r.data?.okunmamis || 0);
        const yeniSonId = (r.data?.son_id || 0);
        const oncekiSonId = sonIdRef.current;
        setCount(yeniSayi);
        if (oncekiSonId > 0 && yeniSonId > oncekiSonId && yeniSayi > 0) {
          beepSes();
          if ('Notification' in window && Notification.permission === 'granted') {
            try { new Notification('📧 Yeni Mail', {body: yeniSayi+' okunmamis mail var', icon:'/favicon.svg', tag:'mr-mail'}); } catch(e) {}
          }
        }
        sonIdRef.current = yeniSonId;
      } catch(e) {}
    };
    tick();
...
const Breadcrumb = ({page, setPage}) => {
  ...
  /* v2.1.3: Mesajlar tüm sekmelerinin breadcrumb'ı */
  if (page.startsWith('mesajlar') && parts.length <= 1) {
    if (page === 'mesajlar-sistem') {
      parts.push({label: 'SİSTEM', id: 'sistem'});
      parts.push({label: 'SİSTEM BİLDİRİMLERİ', id: page});
    } else {
      parts.push({label: 'MESAJLAR', id: 'mesajlar'});
      const sub = page.replace('mesajlar-', '') || 'gelen';
      const labels = {gelen: 'GELEN KUTUSU', giden: 'GİDEN KUTUSU', yeni: 'YENİ MESAJ', sistem: 'SİSTEM BİLDİRİMLERİ'};
      if (sub !== 'mesajlar' && labels[sub]) {
        parts.push({label: labels[sub], id: page});
      }
    }
  }
...
const PageRouter = ({page, setPage, user, setUser}) => {
  ...
  if (page === 'is-paydaslari') {
    return <MR.IsPaydaslariPage setPage={setPage} user={user}/>;
  }

  /* PAYDAŞLAR > EKSPER & SİGORTA (Madde 1 paketinde tanımlı) */
  if (page === 'paydaslar-eksper-sigorta') {
  ...
  /* PERSONEL (geriye dönük uyumluluk — eski linkler için) */
  ...
  /* MESAJLAR — tüm sekmeler (gelen / giden / yeni / sistem)
     v2.1.3: Daha önce sadece 'mesajlar-sistem' tanımlıydı, diğer sekmeler
     (gelen/giden/yeni) boş sayfa açıyordu. */
  if (page.startsWith('mesajlar')) {
    /* v2.9.1: AJANDA artık BİLDİRİMLER sayfasının ilk sekmesi → varsayılan */
    let sub = page === 'mesajlar' ? 'ajanda' : page.replace('mesajlar-', '');
    if (!['ajanda', 'gelen', 'giden', 'yeni', 'sistem'].includes(sub)) sub = 'ajanda';
    return <MR.MesajlarPage setPage={setPage} user={user} subPage={sub}/>;
  }
  /* v2.9.1: Eski 'ajanda' route'unu BİLDİRİMLER > AJANDA sekmesine yönlendir */
  if (page === 'ajanda') {
    return <MR.MesajlarPage setPage={setPage} user={user} subPage="ajanda"/>;
  }
...
const App = () => {
  ...
  const logout = async () => {
    try { if (api.logout) await api.logout(); } catch(e) {}
  ...
  return (
    <div style={{minHeight: '100vh', background: C.bg, color: C.text, position: 'relative', overflow: 'hidden'}}>
      ...
      {/* v2.1.1 GELEN ÇAĞRI WIDGET — sağ üstte popup. Yetki kontrolü widget içinde.
          MASTER v3.0.11 paketlerinde unutulmuştu, geri eklendi. */}
