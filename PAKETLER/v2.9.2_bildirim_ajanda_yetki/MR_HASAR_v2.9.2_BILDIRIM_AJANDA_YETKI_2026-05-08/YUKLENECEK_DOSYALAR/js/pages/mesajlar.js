...
MR.MesajlarPage = ({setPage, user, subPage}) => {
  ...
  // ═══ SEKME TANIMLARI (v2.9.1: AJANDA en üst sekme olarak eklendi) ═══
  const sekmeler = [
  ...
  return (
    <div className="fade-in">
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      {/* v2.9.1: AJANDA sekmesi — embed MR.AjandaPage */}
      {aktifSekme === 'ajanda' && (
        MR.AjandaPage
          ? <MR.AjandaPage setPage={setPage} user={user}/>
          : <div style={{padding:40,textAlign:'center',color:C.textSec}}>Ajanda modülü yüklenmemiş.</div>
      )}
      {aktifSekme === 'gelen' && (
        <div style={S.card}>
                ...
                <thead>
                  <tr style={{background:C.bgHover}}>
                    ...
                    {['','GÖNDEREN','KONU','ÖNCELİK','TARİH','DURUM'].map(h =>
                      <th key={h||'dot'} style={{padding:'10px 12px',textAlign:'left',color: MR.tema==='koyu' ? '#cbd5e1' : C.textMuted,fontWeight:800,fontSize:11,borderBottom:`2px solid ${C.border}`,letterSpacing:0.4}}>{h}</th>
                    )}
                  </tr>
                </thead>
                <tbody>
                ...
                <thead>
                  <tr style={{background:C.bgHover}}>
                    ...
                    {['ALICI','KONU','ÖNCELİK','TARİH','OKUNDU','İŞLEM'].map(h =>
                      <th key={h} style={{padding:'10px 12px',textAlign:'left',color: MR.tema==='koyu' ? '#cbd5e1' : C.textMuted,fontWeight:800,fontSize:11,borderBottom:`2px solid ${C.border}`,letterSpacing:0.4}}>{h}</th>
                    )}
                  </tr>
                </thead>
                <tbody>
