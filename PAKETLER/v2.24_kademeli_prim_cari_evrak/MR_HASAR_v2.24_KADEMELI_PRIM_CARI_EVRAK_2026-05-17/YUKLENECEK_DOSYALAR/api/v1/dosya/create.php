 ...
 *
 * v2.18: Mağdur INSERT'inden TAMAMEN kaldırıldı — telefon2, email, adres,
 * il, ilce, cinsiyet, meslek. DB kolonları yerinde kalır, yeni dosyalarda
 * NULL kaydedilir.
 *
 * v2.17: Frontend hem yeni anahtarları (ma_ruhsat_sahibi, ma_tc_kimlik, ma_model_yili,
 * ma_belge_tescil_no, ma_onarim_gun_suresi, ka_ruhsat_sahibi, ka_tc_kimlik, ka_model_yili,
 * ka_trafik_sirket, ka_belge_tescil_no, ka_ruhsat_sahibi_surucu, ka_surucu_tc_kimlik) hem
 * eski anahtarları (ma_ruhsat, ma_tc, ma_yil, ma_belge_tescil, ma_onarim_gun, ka_ruhsat,
 * ka_tc, ka_yil, ka_trafik, ka_belge_tescil, ka_ruhsat_surucu, ka_surucu_tc) destekler.
 * Yeni anahtar varsa öncelikli, yoksa eski anahtara fallback. DB'de doğru kolonlara
 * (ruhsat_sahibi, tc_kimlik, model_yili, ...) yazar — DOSYA DETAY DÜZENLE ile uyumlu.
 * BH araç INSERT'i artık kasko alanını da yazıyor.
 */
...
try {
    ...
    // 3. Mağdur kaydı (v2.18: telefon2, email, adres, il, ilce, cinsiyet, meslek
    // KESINLIKLE kaldırıldı — bu kolonlar DB'de mevcut kalır ama yeni dosyalarda NULL kaydedilir)
    $stmt = $db->prepare('INSERT INTO magdurlar (dosya_id, tc_kimlik, ad_soyad, telefon, iban, dogum_tarihi, gelir_durumu, gelir_tutari) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $dosyaId,
        ...
        !empty($body['gelir_tutari']) ? (float)$body['gelir_tutari'] : null
    ]);

    // 4. Araç kayıtları (ADK veya MDK ise)
    // v2.17: Frontend hem yeni anahtarlarla hem (geri uyumluluk için) eski anahtarlarla
    // gelen veriyi kabul eder. Yeni anahtar varsa onu, yoksa eski anahtar fallback.
    if ($body['dosya_turu'] === 'ADK' || $body['dosya_turu'] === 'MDK') {
        ...
        if (!empty($body['ma_plaka'])) {
            ...
            $stmt->execute([
                ...
                clean($body['ma_ruhsat_sahibi'] ?? $body['ma_ruhsat'] ?? ''),
                clean($body['ma_tc_kimlik'] ?? $body['ma_tc'] ?? ''),
                clean($body['ma_marka'] ?? ''),
                clean($body['ma_model'] ?? ''),
                !empty($body['ma_model_yili']) ? (int)$body['ma_model_yili'] : (!empty($body['ma_yil']) ? (int)$body['ma_yil'] : null),
                !empty($body['ma_kasko']) ? 1 : 0,
                clean($body['ma_kasko_sirket'] ?? ''),
                clean($body['ma_kasko_police'] ?? ''),
                clean($body['ma_belge_tescil_no'] ?? $body['ma_belge_tescil'] ?? ''),
                !empty($body['ma_onarim_gun_suresi']) ? (int)$body['ma_onarim_gun_suresi'] : (!empty($body['ma_onarim_gun']) ? (int)$body['ma_onarim_gun'] : null),
        ...
        if (!empty($body['ka_plaka'])) {
            ...
            $stmt->execute([
                ...
                clean($body['ka_ruhsat_sahibi'] ?? $body['ka_ruhsat'] ?? ''),
                clean($body['ka_tc_kimlik'] ?? $body['ka_tc'] ?? ''),
                clean($body['ka_marka'] ?? ''),
                clean($body['ka_model'] ?? ''),
                !empty($body['ka_model_yili']) ? (int)$body['ka_model_yili'] : (!empty($body['ka_yil']) ? (int)$body['ka_yil'] : null),
                clean($body['ka_trafik_sirket'] ?? $body['ka_trafik'] ?? ''),
                clean($body['ka_trafik_police'] ?? ''),
                clean($body['ka_belge_tescil_no'] ?? $body['ka_belge_tescil'] ?? ''),
                !empty($body['ka_kasko']) ? 1 : 0,
                !empty($body['ka_ruhsat_sahibi_surucu']) ? clean($body['ka_ruhsat_sahibi_surucu']) : (!empty($body['ka_ruhsat_surucu']) ? clean($body['ka_ruhsat_surucu']) : null),
                clean($body['ka_surucu_ad'] ?? ''),
                clean($body['ka_surucu_tc_kimlik'] ?? $body['ka_surucu_tc'] ?? '')
            ]);
        }
    }
    ...
    if ($body['dosya_turu'] === 'BH') {
        ...
        // BH aracı (v2.17: kasko da kaydediliyor)
        if (!empty($body['bh_arac_plaka'])) {
            $stmt = $db->prepare('INSERT INTO araclar (dosya_id, taraf, plaka, marka, model_yili, kasko) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $dosyaId,
                'magdur',
    ...
    /* v2.24: PERSONEL PRİM KAYDI HOOK — sorumlu personel için cari'ye otomatik prim ekle */
    try {
        require_once __DIR__ . '/../personel/_dosya_prim_hook.php';
        if (function_exists('sync_dosya_prim')) sync_dosya_prim($db, (int)$dosyaId, 'create');
    } catch (\Throwable $e) {
        error_log('[v2.24 dosya/create prim hook] ' . $e->getMessage());
    }
