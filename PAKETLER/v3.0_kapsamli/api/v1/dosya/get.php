...
// v2.1: Soft-delete kontrolü — silinmişse sadece admin görebilir
if (isset($dosya['silindi']) && (int)$dosya['silindi'] === 1 && $user['rol'] !== 'admin') {
...
// v3.0 — Eksper Paydaş bilgisi (eksper_paydas_id varsa)
if (!empty($dosya['eksper_paydas_id'])) {
    try {
        $stmt = $db->prepare('SELECT ad, yetkili, telefon, email, tur FROM paydaslar WHERE id = ?');
        $stmt->execute([$dosya['eksper_paydas_id']]);
        $eks = $stmt->fetch();
        if ($eks) {
            $dosya['eksper_paydas_adi'] = $eks['ad'];
            $dosya['eksper_paydas_yetkili'] = $eks['yetkili'] ?: '';
            $dosya['eksper_paydas_telefon'] = $eks['telefon'] ?: '';
            $dosya['eksper_paydas_email'] = $eks['email'] ?: '';
            $dosya['eksper_paydas_tur'] = $eks['tur'] ?: '';
        }
    } catch (\Exception $e) {}
}

// v3.0 — Dosya kapanış kaydı (varsa snapshot dön)
try {
    $stmtKap = $db->prepare('SELECT * FROM dosya_kapanis_kayitlari WHERE dosya_id = ? LIMIT 1');
    $stmtKap->execute([$dosya['id']]);
    $kap = $stmtKap->fetch();
    if ($kap) {
        $dosya['kapanis_kaydi'] = $kap;
    }
} catch (\Exception $e) {}
