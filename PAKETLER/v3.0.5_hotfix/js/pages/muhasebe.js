...
/* ═══════════════════════════════════════════════════════════
   SEKME 7 – AYLIK MUTABAKAT (v3.0 — Avukat bazlı 3 bölümlü rapor)
   ═══════════════════════════════════════════════════════════ */
const AylikMutabakat = ({setPage, user}) => {
  const {C, S, LIcon, SectionTitle, api} = MR;
  ...
  useEffect(() => {
    api.ortakList({durum:'aktif', limit:200}).then(r => {
      if (r?.success) {
        ...
        setOrtaklar(items);
        if (items.length === 1) setAvukatId(String(items[0].id));
      }
    });
  }, []);
  ...
  const raporAc = () => {
    setError('');
    if (!avukatId) { setError('AVUKAT SEÇMENİZ GEREKLİ'); return; }
    const url = `/api/v1/muhasebe/ay-sonu-rapor-html.php?donem=${yil}-${ay}&avukat_ortak_id=${avukatId}`;
  ...
  const seciliAvukat = ortaklar.find(o => String(o.id) === String(avukatId));

  return (
    <div className="fade-in">
      <div style={{...S.card, marginBottom:20}}>
        <SectionTitle icon="FileSpreadsheet" title="AYLIK MUTABAKAT RAPORU" sub="AVUKAT BAZLI 3 BÖLÜMLÜ CARİ HESAP RAPORU (MADDE 5.4 ENTEGRE)"/>
        <div style={{padding:'18px 20px'}}>

          {error && (
            <div style={{padding:10,background:`${C.danger}22`,borderRadius:8,marginBottom:14,fontSize:12,color:C.danger,fontWeight:600,border:`1px solid ${C.danger}44`,display:'flex',alignItems:'center',gap:8}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 2fr auto',gap:12,alignItems:'flex-end'}}>
            ...
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>AVUKAT (İŞ ORTAĞI)</label>
              <select value={avukatId} onChange={e=>setAvukatId(e.target.value)} style={{...S.select,padding:'10px 12px',fontSize:13}}>
                <option value="">SEÇİNİZ</option>
                {ortaklar.map(o => (
                  <option key={o.id} value={o.id}>{o.ad_soyad}{o.firma ? ` — ${o.firma}` : ''}</option>
                ))}
              </select>
            </div>
            <button onClick={raporAc} style={{...S.btn,...S.btnP,padding:'12px 22px',fontSize:13,fontWeight:800,whiteSpace:'nowrap'}}>
              <LIcon name="ExternalLink" size={14} color="#fff"/> RAPORU AÇ
            </button>
          </div>

          {seciliAvukat && (
            <div style={{marginTop:14,padding:12,background:`${C.purple}11`,borderRadius:8,border:`1px solid ${C.purple}33`,display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:36,height:36,borderRadius:8,background:`${C.purple}22`,display:'flex',alignItems:'center',justifyContent:'center'}}>
                <LIcon name="Scale" size={18} color={C.purple}/>
              </div>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:700}}>{seciliAvukat.ad_soyad}</div>
                <div style={{fontSize:10,color:C.textSec}}>
                  {seciliAvukat.firma ? `${seciliAvukat.firma} • ` : ''}{seciliAvukat.baro || ''}
                  {seciliAvukat.odeme_orani ? ` • ÖDEME ORANI: %${seciliAvukat.odeme_orani}` : ''}
                </div>
              </div>
            </div>
          )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:20}}>
        <div style={{...S.card,padding:16,borderLeft:`4px solid ${C.warning}`}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <div style={{width:28,height:28,borderRadius:6,background:`${C.warning}22`,display:'flex',alignItems:'center',justifyContent:'center'}}>
              <LIcon name="Clipboard" size={14} color={C.warning}/>
            </div>
            <span style={{fontSize:12,fontWeight:800,color:C.warning}}>BÖLÜM 1</span>
          </div>
          <div style={{fontSize:13,fontWeight:700,marginBottom:4}}>DOSYA MASRAFI HESABI</div>
          <div style={{fontSize:11,color:C.textSec,lineHeight:1.5}}>O ay açılan dosyaların açılış masrafları + önceki ay devir + ay içi avukat ödemeleri.</div>
        </div>
        <div style={{...S.card,padding:16,borderLeft:`4px solid ${C.cyan}`}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <div style={{width:28,height:28,borderRadius:6,background:`${C.cyan}22`,display:'flex',alignItems:'center',justifyContent:'center'}}>
              <LIcon name="Scale" size={14} color={C.cyan}/>
            </div>
            <span style={{fontSize:12,fontWeight:800,color:C.cyan}}>BÖLÜM 2</span>
          </div>
          <div style={{fontSize:13,fontWeight:700,marginBottom:4}}>ORTAK MASRAFLAR</div>
          <div style={{fontSize:11,color:C.textSec,lineHeight:1.5}}>SBM Sorgu, Ekran Ödeme, ATK paylaşımlı kalemler. SBM/Ekran ½, ATK tamamı MR alacağı.</div>
        </div>
        <div style={{...S.card,padding:16,borderLeft:`4px solid ${C.success}`}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <div style={{width:28,height:28,borderRadius:6,background:`${C.success}22`,display:'flex',alignItems:'center',justifyContent:'center'}}>
              <LIcon name="Briefcase" size={14} color={C.success}/>
            </div>
            <span style={{fontSize:12,fontWeight:800,color:C.success}}>BÖLÜM 3</span>
          </div>
          <div style={{fontSize:13,fontWeight:700,marginBottom:4}}>KAPANAN DOSYALAR</div>
          <div style={{fontSize:11,color:C.textSec,lineHeight:1.5}}>O ay kapanan dosyaların onur payı toplamı + mahsup/zarar kesintileri (Madde 5.4).</div>
        </div>
      </div>

      <div style={{...S.card,padding:18,background:`${C.accent}06`,borderLeft:`4px solid ${C.accent}`}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6}}>
          <LIcon name="Info" size={16} color={C.accent}/>
          <span style={{fontSize:13,fontWeight:800,color:C.accent}}>NASIL ÇALIŞIR</span>
        </div>
        <ol style={{fontSize:12,color:C.textSec,lineHeight:1.8,paddingLeft:24}}>
          <li><strong>RAPORU AÇ</strong> butonuna basınca yeni sekmede rapor açılır.</li>
          <li>Üstteki form alanlarına manuel kalemleri gir: <strong>önceki ay devir, ay içi ödemeler, SBM, ekran ödeme, ATK</strong>.</li>
          <li>Dosya listesi ve kapanış kayıtları sistemden otomatik gelir; istersen düzeltebilirsin.</li>
          <li>Form alanlarını değiştirdiğinde rapor altta <strong>canlı güncelenir</strong>.</li>
          <li>Sağ üstteki <strong>🖨️ YAZDIR / PDF</strong> butonu ile çıktı al — sadece rapor yazdırılır, form gizlenir.</li>
        </ol>
      </div>
    </div>
  );
