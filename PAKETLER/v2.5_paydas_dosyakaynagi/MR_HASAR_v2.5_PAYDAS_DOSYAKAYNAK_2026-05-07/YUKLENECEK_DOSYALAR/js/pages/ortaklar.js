                ...
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:900}}>
                  ...
                  <tbody>
                    {ortaklar.map((ortak, i) => (
                      <tr key={ortak.id || i} style={{borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',background: secililer.includes(ortak.id) ? `${C.accent}18` : MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),backgroundImage:!secililer.includes(ortak.id)&&MR.tema==='koyu'?(i%2===0?'linear-gradient(90deg, #111827 0%, #0d1321 100%)':'linear-gradient(90deg, #0d1321 0%, #090e1a 100%)'):'none',borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.03)':'0 1px 4px rgba(99,102,241,0.08)',transition:'all .2s ease',borderRadius:8}}
                      ...
                      {hareketler.map((h, i) => {
                        ...
                        return (
                          <tr key={h.id || i} style={{borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',background:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',borderRadius:8,transition:'all .2s ease'}}
                            ...
                            <td style={{padding:'8px 12px',color:MR.tema==='koyu'?'#e2e8f0':'#1e293b',fontSize:'12px',fontWeight:600}}>{MR.tarihFmt ? MR.tarihFmt(h.tarih || h.created_at) : '-'}</td>
...
const IsPaydaslari = ({setPage, user}) => {
  ...
  /* ─── TÜR EŞLEŞTİRME ─── (v2.5: 5 yeni tip + eski tipler korunur) */
  const turMap = {
    // v2.5 yeni tipler
    'servis':           {label:'SERVİS',            renk: C.warning, icon:'Wrench'},
    'kaportaci':        {label:'KAPORTACI',         renk: C.danger,  icon:'Hammer'},
    'acente':           {label:'ACENTE',            renk: C.accent,  icon:'Shield'},
    'pasif_temsilci':   {label:'PASİF TEMSİLCİ',    renk: C.purple,  icon:'UserCheck'},
    // eski tipler (geri uyumluluk)
  ...
  /* ─── FORM ─── */
  const bosForm = {ad:'', tur:'servis', yetkili:'', telefon:'', email:'', adres:'', il:'', prim_adk:'', prim_bh:'', durum:'aktif', notlar:''};
  const [form, setForm] = useState({...bosForm});
  const up = (k, v) => setForm(p => ({...p, [k]: v}));
  ...
  return (
    <div>
                ...
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:900}}>
                  ...
                  <tbody>
                    {paydaslar.map((paydas, i) => (
                      <tr key={paydas.id || i} onClick={() => { setSeciliPaydas(paydas); setKartModal(true); }} style={{borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',background: secililerP.includes(paydas.id) ? `${C.accent}18` : MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',borderRadius:8,transition:'all .2s ease',cursor:'pointer'}}
                      ...
                      {komisyonlar.map((k, i) => {
                        ...
                        return (
                          <tr key={k.id || i} style={{borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',background:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',borderRadius:8,transition:'all .2s ease'}}
                            onMouseEnter={e => {if(MR.tema==='koyu'){e.currentTarget.style.borderLeft='3px solid rgba(6,182,212,0.8)';e.currentTarget.style.boxShadow='0 4px 16px rgba(6,182,212,0.15)';}else{e.currentTarget.style.borderLeft='3px solid rgba(99,102,241,0.6)';e.currentTarget.style.boxShadow='0 4px 12px rgba(99,102,241,0.15)';}e.currentTarget.style.transform='translateY(-1px)';}}
                            onMouseLeave={e => {e.currentTarget.style.borderLeft=MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)';e.currentTarget.style.boxShadow=MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)';e.currentTarget.style.transform='translateY(0)';}}>
                            <td style={{padding:'8px 12px',color:MR.tema==='koyu'?'#e2e8f0':'#1e293b',fontSize:'12px',fontWeight:600}}>{MR.tarihFmt ? MR.tarihFmt(k.tarih || k.created_at) : '-'}</td>
      ...
      {/* v2.4: PAYDAŞ KART MODAL */}
      {MR.PaydasKartModal && <MR.PaydasKartModal
        open={kartModal}
        onClose={() => { setKartModal(false); setSeciliPaydas(null); }}
        paydas={seciliPaydas}
        user={user}/>}
    </div>
  );
...
/* v2.4 NOT: Paydaş kart modal'ı PaydaslarPage component'ine eklenmeli.
   Ancak ortaklar.js iki ayrı component içeriyor. Modal entegrasyonu
   PaydaslarPage'in JSX'inde yapılır.

   Eğer otomatik enjekte edilmesi gerekirse, PaydaslarPage return'ünde
   en son </div>'tan ÖNCE şu satırı manuel ekleyin:
...
/* ═══════════════════════════════════════════════════════════
   v2.5 — Component'leri MR namespace'ine expose et
   is-ortaklari.js ve is-paydaslari.js sayfaları bunları reuse eder
   ═══════════════════════════════════════════════════════════ */
MR.IsOrtaklariView  = IsOrtaklari;
MR.IsPaydaslariView = IsPaydaslari;
