  ...
  return (
    React.createElement('div', {className:'fade-in'},
      ...
      React.createElement('div', {style:{display:'flex',gap:12,marginBottom:20,flexWrap:'wrap',alignItems:'flex-end'}},
        ...
        data && React.createElement('button', {onClick:yazdir, className:'no-print', style:Object.assign({},S.btn,{fontSize:12,padding:'10px 18px',background:'linear-gradient(180deg, #34d399 0%, #10b981 40%, #059669 100%)',color:'#fff',fontWeight:800,border:'none',boxShadow:'0 4px 14px -2px rgba(16,185,129,0.55), 0 2px 4px -1px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.25)',borderBottom:'2px solid #047857'})},
          React.createElement(LIcon, {name:'Printer', size:14, color:'#fff'}), ' YAZDIR'
        ),
        /* v2.9.3: PDF İNDİR butonu — html2pdf üzerinden gerçek PDF dosyası */
        data && React.createElement('button', {onClick:function(){
          var el = document.getElementById('aysonu-rapor-icerik');
          if (!el) { alert('Rapor içeriği bulunamadı'); return; }
          if (!window.html2pdf) { alert('html2pdf kütüphanesi yüklenmemiş'); return; }
          var dosyaAdi = 'AY_SONU_RAPORU_' + (data.donem && data.donem.ay_adi ? data.donem.ay_adi.replace(/\s+/g,'_') : ay) + '.pdf';
          window.html2pdf().set({
            margin: [10, 10, 10, 10],
            filename: dosyaAdi,
        ...
        }, className:'no-print', style:Object.assign({},S.btn,{fontSize:12,padding:'10px 18px',background:'#dc2626',color:'#fff',fontWeight:800,border:'none'})},
          React.createElement(LIcon, {name:'FileDown', size:14, color:'#fff'}), ' PDF İNDİR'
        ),
        /* v2.9.3: ÖNİZLE — yeni pencerede temiz HTML olarak göster */
        data && React.createElement('button', {onClick:function(){
          var el = document.getElementById('aysonu-rapor-icerik');
          if (!el) { alert('Rapor içeriği bulunamadı'); return; }
          var w = window.open('', '_blank', 'width=900,height=1100');
          if (!w) { alert('Yeni pencere açılamadı (popup engelli olabilir)'); return; }
          var baslik = 'AY SONU RAPORU — ' + ((data.donem && data.donem.ay_adi) || ay);
          var css = 'body{font-family:"Segoe UI",Tahoma,Arial,sans-serif;padding:24px 32px;color:#1a1a2e;background:#fff;font-size:12px;line-height:1.55}'
                  + 'table{width:100%;border-collapse:collapse;margin:8px 0}'
                  + 'th,td{border:1px solid #bbb;padding:6px 8px;text-align:left;font-size:11px;color:#1a1a2e}'
                  + 'th{background:#eef0f4;font-weight:700}'
                  + 'button,.no-print{display:none!important}'
                  + 'svg{display:none}'
                  + '*{box-shadow:none!important;text-shadow:none!important}'
                  + '@media print{body{padding:10px}@page{size:A4;margin:1cm}}';
          w.document.write('<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><title>' + baslik + '</title><style>' + css + '</style></head><body><h2 style="margin:0 0 16px">' + baslik + '</h2>' + el.innerHTML + '<' + 'script>setTimeout(function(){window.print()},500);</' + 'script></body></html>');
          w.document.close();
        }, className:'no-print', style:Object.assign({},S.btn,{fontSize:12,padding:'10px 18px',background:'#0ea5e9',color:'#fff',fontWeight:800,border:'none'})},
          React.createElement(LIcon, {name:'Eye', size:14, color:'#fff'}), ' ÖNİZLE'
        )
      ),
