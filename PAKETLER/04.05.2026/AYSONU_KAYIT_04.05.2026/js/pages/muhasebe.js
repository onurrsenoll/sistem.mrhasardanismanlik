...
const GiderYonetimi = ({setPage, user}) => {
  ...
  /* PERSONEL MAAŞ ÖZELLİĞİ */
  const [personelListesi, setPersonelListesi] = useState([]);
  useEffect(() => { api.personelList({durum:'aktif'}).then(r => { if(r?.success) setPersonelListesi(r.data?.items || r.data || []); }); }, []);

  const personelMaasKategorileri = ['PERSONEL GİDERİ','PERSONEL MAAŞ','PERSONEL MAAŞI'];
  const isPersonelMaas = personelMaasKategorileri.includes(form.kategori);

  const handlePersonelSecim = async (pid) => {
    if (!pid) { setForm(p => ({...p, personel_id:''})); return; }
    const pers = personelListesi.find(p => String(p.id) === pid);
    if (!pers) return;
    /* Önce hakedis verisi ara */
    let tutar = pers.maas || 0;
    try {
      const r = await api.hakedisList({personel_id: pid});
      if (r?.success) {
    ...
    setForm(p => ({...p, personel_id: pid, tutar: fmtInput(String(tutar)), aciklama: pers.ad_soyad + ' - MAAŞ/HAKEDİŞ ÖDEMESİ'}));
  };
  ...
  const [giderKategorileri, setGiderKategorileri] = useState(['OFİS GİDERİ','PERSONEL GİDERİ','PERSONEL MAAŞ','ULAŞIM','YAKIT','BİLİRKİŞİ ÜCRETİ','AVUKAT ÜCRETİ','KİRA','VERGİ / HARÇ','DOSYA MASRAFI','DİĞER GİDER']);
      ...
      <Modal open={modalAcik} onClose={() => setModalAcik(false)} title="YENİ GİDER KAYDI" width="560px">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
          ...
          <FormGroup label="KATEGORİ *">
            <select style={S.select} value={form.kategori} onChange={e => { setForm(p => ({...p, kategori: e.target.value, personel_id:''})); }}>
              {giderKategorileri.map(k => <option key={k} value={k}>{k}</option>)}
            </select>
          </FormGroup>
          {isPersonelMaas && (
            <FormGroup label="PERSONEL SEÇİMİ *">
              <select style={{...S.select,borderColor:C.accent}} value={form.personel_id || ''} onChange={e => handlePersonelSecim(e.target.value)}>
                <option value="">PERSONEL SEÇİNİZ</option>
                {personelListesi.map(p => <option key={p.id} value={p.id}>{p.ad_soyad} ({fmt(p.maas)})</option>)}
              </select>
            </FormGroup>
          )}
          <FormGroup label="TUTAR *">
        ...
        <div style={{overflowX:'auto'}}>
          {komisyonlar.length === 0 ? (
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:900}}>
              ...
              <tbody>
                {komisyonlar.map((k, i) => (
                  <tr key={k.id || i} style={{backgroundColor:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',transition:'all .2s ease',borderRadius:8}}
                    onMouseEnter={e=>{if(MR.tema==='koyu'){e.currentTarget.style.borderLeft='3px solid rgba(6,182,212,0.8)';e.currentTarget.style.boxShadow='0 4px 16px rgba(6,182,212,0.15)';}else{e.currentTarget.style.borderLeft='3px solid rgba(99,102,241,0.6)';e.currentTarget.style.boxShadow='0 4px 12px rgba(99,102,241,0.15)';}e.currentTarget.style.transform='translateY(-1px)';}}
                    onMouseLeave={e=>{e.currentTarget.style.backgroundColor=MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff');e.currentTarget.style.borderLeft=MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)';e.currentTarget.style.boxShadow=MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)';e.currentTarget.style.transform='translateY(0)';}}>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:900}}>
                ...
                <tbody>
                  {veriler.map((d, i) => {
                    ...
                    const pozitif = net >= 0;
                    return (
                      <tr key={d.dosya_id || i} style={{backgroundColor:pozitif?(MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff')):(MR.tema==='koyu'?'#3b1a1a':'#fff0f0'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',transition:'all .2s ease',borderRadius:8}}
                        onMouseEnter={e=>{if(MR.tema==='koyu'){e.currentTarget.style.borderLeft='3px solid rgba(6,182,212,0.8)';e.currentTarget.style.boxShadow='0 4px 16px rgba(6,182,212,0.15)';}else{e.currentTarget.style.borderLeft='3px solid rgba(99,102,241,0.6)';e.currentTarget.style.boxShadow='0 4px 12px rgba(99,102,241,0.15)';}e.currentTarget.style.transform='translateY(-1px)';}}
                        onMouseLeave={e=>{e.currentTarget.style.backgroundColor=pozitif?(MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff')):(MR.tema==='koyu'?'#3b1a1a':'#fff0f0');e.currentTarget.style.borderLeft=MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)';e.currentTarget.style.boxShadow=MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)';e.currentTarget.style.transform='translateY(0)';}}>
                        <td style={{padding:'10px 12px'}}>
                ...
                <tfoot>
                  <tr style={{background:MR.tema==='koyu'?'#0f2342':'#1e40af22',borderTop:`2px solid ${C.border}`}}>
                    ...
                    {seciliAvukat === gi && (
                      <div style={{overflowX:'auto'}}>
                        <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                          ...
                          <tbody>
                            {(g.dosyalar || []).map((d, di) => (
                              <tr key={di} style={{backgroundColor:MR.tema==='koyu'?(di%2===0?'#111827':'#0d1321'):(di%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',transition:'all .2s ease',borderRadius:8}}
                                onMouseEnter={e=>{if(MR.tema==='koyu'){e.currentTarget.style.borderLeft='3px solid rgba(6,182,212,0.8)';e.currentTarget.style.boxShadow='0 4px 16px rgba(6,182,212,0.15)';}else{e.currentTarget.style.borderLeft='3px solid rgba(99,102,241,0.6)';e.currentTarget.style.boxShadow='0 4px 12px rgba(99,102,241,0.15)';}e.currentTarget.style.transform='translateY(-1px)';}}
                                onMouseLeave={e=>{e.currentTarget.style.backgroundColor=MR.tema==='koyu'?(di%2===0?'#111827':'#0d1321'):(di%2===0?'#ffffff':'#f0f4ff');e.currentTarget.style.borderLeft=MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)';e.currentTarget.style.boxShadow=MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)';e.currentTarget.style.transform='translateY(0)';}}>
...
const AySonuRaporu = ({setPage, user}) => {
  const {C, S, LIcon, StatCard, Badge, SectionTitle, Loading, EmptyState, api, fmt, Modal, FormGroup} = MR;
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [ay, setAy] = useState(new Date().toISOString().slice(0,7));
  const [ortakId, setOrtakId] = useState('');
  const [ortaklar, setOrtaklar] = useState([]);
  // v3.0.12 — Kaydet/Liste/Düzenle/Sil
  const [kayitListesi, setKayitListesi] = useState([]);
  const [listeModal, setListeModal] = useState(false);
  const [kaydedilen, setKaydedilen] = useState(null); // {id, durum, baslik}
  ...
  const yukle = async () => {
    ...
    if (r?.success) {
      ...
      // Bu dönem için kayıtlı rapor var mı bak
      try {
        const lr = await apiCall('/api/v1/muhasebe/aysonu-list.php');
        if (lr?.success) {
          const items = lr.data?.items || [];
          setKayitListesi(items);
          const mevcut = items.find(x => x.donem === ay && String(x.ortak_id||'') === String(ortakId||''));
          if (mevcut) setKaydedilen({id: mevcut.id, durum: mevcut.durum, baslik: mevcut.baslik});
          else setKaydedilen(null);
        }
      } catch(e) {}
    }
    setLoading(false);
  ...
  // v3.0.12 — Snapshot kaydet (UPSERT)
  const snapshotKaydet = async (yeniDurum) => {
    if (!data) return;
    setKayitYukleniyor(true);
    setKayitMesaj(null);
    const payload = {
      donem: ay,
      ortak_id: ortakId ? parseInt(ortakId) : null,
      durum: yeniDurum || (kaydedilen?.durum === 'kesin' ? 'kesin' : 'taslak'),
      bolum1: data.bolum1 ? {
        dosya_sayisi: data.bolum1.dosya_sayisi || 0,
        toplam_masraf: data.bolum1.toplam_masraf || 0,
        devir: data.bolum1.devir || 0,
        odemeler: data.bolum1.odemeler || 0,
        alacak: data.bolum1.ortak_alacak || data.bolum1.alacak || 0
      } : {},
      bolum2: data.bolum2 ? {
        kapanan_sayisi: data.bolum2.kapanan_sayisi || 0,
        toplam_kazanc: data.bolum2.toplam_kazanc || 0,
        mahsup: data.bolum2.mahsup || 0,
        net: data.bolum2.net || 0
      } : {},
      final_bakiye: data.final?.bakiye || 0,
      rapor_data: data,
      not_metni: notMetni
    };
    const r = await apiCall('/api/v1/muhasebe/aysonu-kaydet.php', {method:'POST', body: JSON.stringify(payload)});
    setKayitYukleniyor(false);
    if (r?.success) {
      setKayitMesaj({tip:'success', text: r.message || 'Kaydedildi'});
      setKaydedilen({id: r.data.id, durum: r.data.durum, baslik: r.data.baslik});
      // Listeyi tazele
      const lr = await apiCall('/api/v1/muhasebe/aysonu-list.php');
      if (lr?.success) setKayitListesi(lr.data?.items || []);
      setTimeout(() => setKayitMesaj(null), 3000);
    } else {
      setKayitMesaj({tip:'error', text: r?.error || 'Kayıt hatası'});
    }
  };

  // v3.0.12 — Kaydı yükle (DÜZENLE)
  const kayitYukle = async (id) => {
    const r = await apiCall('/api/v1/muhasebe/aysonu-getir.php?id=' + id);
    if (r?.success) {
      const k = r.data;
      setAy(k.donem);
      setOrtakId(k.ortak_id || '');
      setNotMetni(k.not_metni || '');
      setKaydedilen({id: k.id, durum: k.durum, baslik: k.baslik});
      setListeModal(false);
      setKayitMesaj({tip:'info', text: k.baslik + ' yüklendi - canlı veri ile güncellenmektedir'});
      setTimeout(() => setKayitMesaj(null), 4000);
    }
  };

  // v3.0.12 — Sil
  const kayitSil = async (id) => {
    const r = await apiCall('/api/v1/muhasebe/aysonu-sil.php', {method:'POST', body: JSON.stringify({id})});
    if (r?.success) {
      const lr = await apiCall('/api/v1/muhasebe/aysonu-list.php');
      if (lr?.success) setKayitListesi(lr.data?.items || []);
      if (kaydedilen?.id === id) setKaydedilen(null);
      setSilOnay(null);
      setKayitMesaj({tip:'success', text: r.message});
      setTimeout(() => setKayitMesaj(null), 3000);
    } else {
      setKayitMesaj({tip:'error', text: r?.error || 'Silme hatası'});
    }
  };
  ...
  return (
    React.createElement('div', {className:'fade-in'},
      ...
      React.createElement('div', {style:{display:'flex',gap:12,marginBottom:20,flexWrap:'wrap',alignItems:'flex-end'}},
        ...
        /* v3.0.12 — KAYDET butonu (snapshot) */
        data && React.createElement('button', {onClick:function(){snapshotKaydet()}, disabled: kayitYukleniyor || kaydedilen?.durum === 'kesin',
          style:Object.assign({},S.btn,{fontSize:12,padding:'10px 18px',background: kaydedilen?.durum === 'kesin' ? '#9ca3af' : '#3498DB',color:'#fff',fontWeight:800,border:'none',cursor: kaydedilen?.durum === 'kesin' ? 'not-allowed' : 'pointer'})},
          React.createElement(LIcon, {name: kaydedilen?.durum === 'kesin' ? 'Lock' : 'Save', size:14, color:'#fff'}),
          ' ' + (kayitYukleniyor ? 'KAYDEDİLİYOR...' : (kaydedilen?.durum === 'kesin' ? 'KESİN' : (kaydedilen ? 'GÜNCELLE' : 'KAYDET')))
        ),
        /* v3.0.12 — KESİNLEŞTİR butonu (sadece taslak ise) */
        data && kaydedilen && kaydedilen.durum === 'taslak' && React.createElement('button', {onClick:function(){snapshotKaydet('kesin')}, disabled: kayitYukleniyor,
          style:Object.assign({},S.btn,{fontSize:12,padding:'10px 14px',background:'#7c3aed',color:'#fff',fontWeight:800,border:'none'})},
          React.createElement(LIcon, {name:'Lock', size:14, color:'#fff'}), ' KESİNLEŞTİR'
        ),
        /* v3.0.12 — KAYITLI RAPORLAR butonu */
        React.createElement('button', {onClick:function(){setListeModal(true)},
          style:Object.assign({},S.btn,{fontSize:12,padding:'10px 18px',background:'#F39C12',color:'#fff',fontWeight:800,border:'none'})},
          React.createElement(LIcon, {name:'Archive', size:14, color:'#fff'}), ' KAYITLI RAPORLAR (' + kayitListesi.length + ')'
        )
      ),

      /* v3.0.12 — Mesaj banner */
      kayitMesaj && React.createElement('div', {
        style:{padding:'10px 14px', borderRadius:8, marginBottom:14, fontSize:12, fontWeight:600,
          background: kayitMesaj.tip==='error' ? (C.danger+'22') : (kayitMesaj.tip==='success' ? (C.success+'22') : (C.accent+'22')),
          color: kayitMesaj.tip==='error' ? C.danger : (kayitMesaj.tip==='success' ? C.success : C.accent),
          border: '1px solid ' + (kayitMesaj.tip==='error' ? C.danger : (kayitMesaj.tip==='success' ? C.success : C.accent)) + '44'}
      }, kayitMesaj.text),

      /* v3.0.12 — Kayıtlı durum + not alanı (rapor varsa) */
      data && kaydedilen && React.createElement('div', {
        style:{padding:'10px 14px', borderRadius:8, marginBottom:14, background: kaydedilen.durum === 'kesin' ? C.purple+'11' : C.warning+'11',
          border:'1px solid ' + (kaydedilen.durum === 'kesin' ? C.purple : C.warning) + '33', display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}
      },
        React.createElement(LIcon, {name: kaydedilen.durum === 'kesin' ? 'Lock' : 'Edit3', size:14, color: kaydedilen.durum === 'kesin' ? C.purple : C.warning}),
        React.createElement('span', {style:{fontSize:12,fontWeight:700}}, 'KAYITLI: ' + kaydedilen.baslik + ' (' + kaydedilen.durum.toUpperCase() + ')')
      ),
      data && React.createElement('div', {style:{marginBottom:14}},
        React.createElement('label', {style:{fontSize:10, color:C.textMuted, fontWeight:600, display:'block', marginBottom:4}}, 'NOT (opsiyonel — kayda eklenir)'),
        React.createElement('textarea', {value: notMetni, onChange:function(e){setNotMetni(e.target.value)}, rows:2,
          style:Object.assign({}, S.input, {width:'100%',resize:'vertical',padding:'8px 10px',fontSize:12})})
      ),
            ...
            React.createElement('table', {style:{width:'100%',borderCollapse:'collapse',maxWidth:400}},
              ...
              React.createElement('tbody', null,
                Object.entries(b1.tur_dagilimi).map(function(e,i){
                  ...
                  return React.createElement('tr', {key:tur, style:{backgroundColor:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderLeft:MR.tema==='koyu'?'3px solid rgba(6,182,212,0.5)':'3px solid rgba(99,102,241,0.4)',borderBottom:MR.tema==='koyu'?'1px solid rgba(6,182,212,0.1)':'1px solid rgba(99,102,241,0.1)',boxShadow:MR.tema==='koyu'?'0 2px 8px rgba(0,0,0,0.3)':'0 1px 4px rgba(99,102,241,0.08)',transition:'all .2s ease',borderRadius:8}},
      ...
      /* v3.0.12 — KAYITLI RAPORLAR MODAL */
      listeModal && React.createElement('div', {
        onClick: function(){setListeModal(false)},
        ...
        React.createElement('div', {
          ...
          style: {background:C.bgCard, padding:0, borderRadius:14, maxWidth:880, width:'90vw', maxHeight:'85vh', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,0.4)'}
        },
          /* Header */
          React.createElement('div', {style:{padding:'14px 20px',borderBottom:'1px solid '+C.border,display:'flex',alignItems:'center',justifyContent:'space-between',background:C.accent+'08'}},
            React.createElement('div', {style:{display:'flex',alignItems:'center',gap:8}},
              React.createElement(LIcon, {name:'Archive', size:16, color:C.accent}),
              React.createElement('span', {style:{fontSize:14,fontWeight:800}}, 'KAYITLI AY SONU RAPORLARI (' + kayitListesi.length + ')')
            ),
            React.createElement('button', {onClick:function(){setListeModal(false)}, style:{background:'transparent',border:0,fontSize:20,cursor:'pointer',color:C.textMuted}}, '×')
          ),
          /* Tablo */
          React.createElement('div', {style:{flex:1, overflow:'auto', padding:'8px 20px 16px'}},
            kayitListesi.length === 0
              ? React.createElement('div', {style:{padding:40,textAlign:'center',color:C.textMuted,fontStyle:'italic'}}, 'Henüz kayıtlı rapor yok. Sayfada raporu görüntüledikten sonra "KAYDET" butonuna tıkla.')
              : React.createElement('table', {style:{width:'100%',borderCollapse:'collapse',fontSize:12}},
                React.createElement('thead', null,
                  React.createElement('tr', {style:{borderBottom:'2px solid '+C.border}},
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'left',fontSize:10,fontWeight:700,color:C.textSec}}, 'DÖNEM'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'left',fontSize:10,fontWeight:700,color:C.textSec}}, 'BAŞLIK'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'left',fontSize:10,fontWeight:700,color:C.textSec}}, 'ORTAK'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'center',fontSize:10,fontWeight:700,color:C.textSec}}, 'DURUM'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'right',fontSize:10,fontWeight:700,color:C.textSec}}, 'BÖLÜM 1'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'right',fontSize:10,fontWeight:700,color:C.textSec}}, 'BÖLÜM 2'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'right',fontSize:10,fontWeight:700,color:C.textSec}}, 'FİNAL'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'left',fontSize:10,fontWeight:700,color:C.textSec}}, 'OLUŞTURAN'),
                    React.createElement('th', {style:{padding:'8px 6px',textAlign:'center',fontSize:10,fontWeight:700,color:C.textSec}}, 'İŞLEM')
                  )
                ),
                React.createElement('tbody', null,
                  kayitListesi.map(function(k){
                    var fNum = parseFloat(k.final_bakiye)||0;
                    return React.createElement('tr', {key:k.id, style:{borderBottom:'1px solid '+C.border, background: kaydedilen?.id === k.id ? C.accent+'11' : 'transparent'}},
                      React.createElement('td', {style:{padding:'8px 6px',fontWeight:700,fontFamily:'monospace'}}, k.donem),
                      React.createElement('td', {style:{padding:'8px 6px',fontSize:11}}, k.baslik),
                      React.createElement('td', {style:{padding:'8px 6px',fontSize:11,color:C.textSec}}, k.ortak_adi || '—'),
                      React.createElement('td', {style:{padding:'8px 6px',textAlign:'center'}},
                        React.createElement('span', {style:{padding:'3px 8px',borderRadius:4,fontSize:10,fontWeight:700,
                          background: k.durum==='kesin' ? C.purple+'22' : C.warning+'22',
                          color: k.durum==='kesin' ? C.purple : C.warning}}, k.durum.toUpperCase())
                      ),
                      React.createElement('td', {style:{padding:'8px 6px',textAlign:'right',fontFamily:'monospace',fontSize:11,color: parseFloat(k.bolum1_alacak)>=0?C.success:C.danger}}, fmt(parseFloat(k.bolum1_alacak)||0)),
                      React.createElement('td', {style:{padding:'8px 6px',textAlign:'right',fontFamily:'monospace',fontSize:11,color: parseFloat(k.bolum2_net)>=0?C.success:C.danger}}, fmt(parseFloat(k.bolum2_net)||0)),
                      React.createElement('td', {style:{padding:'8px 6px',textAlign:'right',fontFamily:'monospace',fontWeight:800,fontSize:13,color: fNum>=0?C.success:C.danger}}, fmt(fNum)),
                      React.createElement('td', {style:{padding:'8px 6px',fontSize:11,color:C.textSec}}, k.olusturan_adi || '—'),
                      React.createElement('td', {style:{padding:'8px 6px',textAlign:'center',whiteSpace:'nowrap'}},
                        React.createElement('button', {onClick:function(){kayitYukle(k.id)},
                        ...
                        user?.rol === 'admin' && React.createElement('button', {onClick:function(){setSilOnay(k)}, disabled: k.durum==='kesin',
                          style:{padding:'5px 10px',fontSize:10,background: k.durum==='kesin' ? C.bgInput : C.danger,color: k.durum==='kesin' ? C.textMuted : '#fff',border:0,borderRadius:4,cursor: k.durum==='kesin' ? 'not-allowed' : 'pointer'}}, 'SİL')
                      )
                    );
                  })
                )
      ...
      /* v3.0.12 — SİL ONAY MODAL */
      silOnay && React.createElement('div', {
        onClick: function(){setSilOnay(null)},
        ...
        React.createElement('div', {
          onClick: function(e){e.stopPropagation()},
          style: {background:C.bgCard, padding:24, borderRadius:12, maxWidth:420, boxShadow:'0 10px 40px rgba(0,0,0,0.4)'}
        },
          React.createElement('div', {style:{fontSize:14,fontWeight:800,marginBottom:8,color:C.danger}}, 'RAPOR SİLİNECEK'),
          React.createElement('div', {style:{fontSize:12,color:C.textSec,marginBottom:16}},
            React.createElement('strong', null, silOnay.baslik), ' kalıcı olarak silinecek. Onaylıyor musunuz?'
          ),
          React.createElement('div', {style:{display:'flex',gap:8,justifyContent:'flex-end'}},
            React.createElement('button', {onClick:function(){setSilOnay(null)},
            ...
            React.createElement('button', {onClick:function(){kayitSil(silOnay.id)},
