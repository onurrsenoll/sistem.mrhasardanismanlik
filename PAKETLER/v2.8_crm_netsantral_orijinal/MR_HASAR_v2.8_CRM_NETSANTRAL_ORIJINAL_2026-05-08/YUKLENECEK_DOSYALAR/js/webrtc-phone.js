...
MR.webrtcTelefon = {
  ...
  ara: async function(numara) {
    ...
    try {
      /* v2.1.1: GELİŞMİŞ MİKROFON AKIŞI — echoCancellation + noiseSuppression
         + autoGainControl + 48kHz HD + seçili mikrofon (kulaklık) + gain.
         Eski düz getUserMedia karşı tarafa eko/gürültü/düşük kalite veriyordu. */
      console.log('[WEBRTC] _mikrofonAkisiOlustur ÇAĞRILIYOR (gelişmiş)...');
      var stream = await this._mikrofonAkisiOlustur();
      if (!stream) {
        this._temizle();
        this._durumBildir('hata', 'MİKROFON ERİŞİMİ BAŞARISIZ');
        return false;
      }
      console.log('[WEBRTC] MİKROFON BAŞARILI ✓ track:', stream.getAudioTracks().length);
  ...
  cevapla: async function() {
    ...
    try {
      /* v2.1.1: GELİŞMİŞ MİKROFON AKIŞI — gelen çağrıyı cevaplarken de
         echoCancellation + noiseSuppression + autoGainControl uygula. */
      console.log('[WEBRTC] CEVAPLA: _mikrofonAkisiOlustur ÇAĞRILIYOR (gelişmiş)...');
      var stream = await this._mikrofonAkisiOlustur();
      if (!stream) {
        this._durumBildir('hata', 'MİKROFON ERİŞİMİ BAŞARISIZ');
        return false;
      }
      console.log('[WEBRTC] CEVAPLA: MİKROFON BAŞARILI ✓ track:', stream.getAudioTracks().length);
