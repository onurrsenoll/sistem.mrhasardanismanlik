...
MR.IhbarFoyuPage = ({ setPage, user }) => {
  ...
  /* ═══ v2.9.3: PDF İNDİR + ÖNİZLE — html2pdf üzerinden ═══ */
  const yazdirilacakAlanId = 'ihbar-foyu-yazdirma-alani';
  const pdfIndir = () => {
    const el = document.getElementById(yazdirilacakAlanId);
    if (!el) { alert('İçerik bulunamadı'); return; }
    if (!window.html2pdf) { alert('html2pdf kütüphanesi yüklenmemiş'); return; }
    const dosyaAdi = 'IHBAR_FOYU_' + (form.plaka || 'rapor').replace(/\s+/g,'_').toUpperCase() + '_' +
  ...
  const onizle = () => {
    ...
    w.document.write(
      ...
      '<title>İHBAR FÖYÜ — ÖNİZLEME — ' + (form.plaka || '') + '</title>' +
      '<style>' +
      'body{font-family:"Segoe UI",Tahoma,Arial,sans-serif;padding:24px 32px;color:#1a1a2e;background:#fff;font-size:12px;line-height:1.55}' +
      ...
      'button,input[type="button"],.no-print{display:none!important}' +
  ...
  /* ═══ RENDER — ANA YAPI ═══ */
  return (
    <div className="fade-in" style={{ padding: '12px 0' }} id={yazdirilacakAlanId}>
      {/* ÜST BAR */}
      <div style={{
      ...
      <div style={{
        ...
        <button className="no-print" style={{ ...S.btn, background: C.warning, color: '#000', fontSize: 12, padding: '10px 18px', borderRadius: 8 }}
          onClick={() => window.print()}>
          <LIcon name="Printer" size={14} color="#000" /> YAZDIR
        </button>
        <button className="no-print" style={{ ...S.btn, background: C.danger, color: '#fff', fontSize: 12, padding: '10px 18px', borderRadius: 8 }}
          onClick={pdfIndir}>
          <LIcon name="FileDown" size={14} color="#fff" /> PDF İNDİR
        </button>
        <button className="no-print" style={{ ...S.btn, background: '#0ea5e9', color: '#fff', fontSize: 12, padding: '10px 18px', borderRadius: 8 }}
          onClick={onizle}>
          <LIcon name="Eye" size={14} color="#fff" /> ÖNİZLE
        </button>
