/**
 * MR HASAR DANIŞMANLIK — DOSYA DETAY (v2.22)
 * v2.22 — LAYOUT FIX + ARAÇ GÖRSEL ENTEGRASYONU (imagin.studio):
 *   ┌─ LAYOUT FIX ────────────────────────────────────────────
 *   │ • FULL-BLEED: sayfanın sağ-sol boşlukları kapatıldı,
 *   │   container max-width sınırı aşıldı (3 sütun grid genişledi)
 *   │ • Aşama dropdown sabit 200px (taşma yok)
 *   │ • 3 sütun grid: her sütun minmax(320px, 1fr) eşit pay
 *   │ • InfoRow value: maxWidth:200 → flex:1 (sütun genişliğine uyum)
 *   │ • AracGorsel sağ kolon başlığı kısaltıldı: "ARAÇ SAHİBİ"
 *   └─────────────────────────────────────────────────────────
 *   ┌─ ARAÇ GÖRSEL EŞLEŞTİRME MODÜLÜ ─────────────────────────
 *   │ • imagin.studio CDN'den marka+model+yıl ile profesyonel
 *   │   araç görseli otomatik gelir (Mağdur & Karşı araç için)
 *   │ • Görsel kart ÜSTÜNDE 160px banner olarak gösterilir
 *   │ • Yüklenemezse fallback: marka adı + model + yıl tipografisi
 *   │ • localStorage cache (mr_arac_gorsel_v1):
 *   │   { MARKA_MODEL_YIL: "ok" | "fail" } — fail kayıtlarda
 *   │   tekrar denenmez (network tasarrufu)
 *   │ • Marka logosu sağ alt köşede minik (logo.clearbit.com)
 *   │ ⚠ Üretim için imagin.studio'dan LİSANSLI API key alınmalı.
 *   │   Şu an demo key kullanılıyor: 'hrjavascript-mastery'
 *   │   IMAGIN_API_KEY sabitini değiştirmek yeterli.
 *   └─────────────────────────────────────────────────────────
 *
 * v2.21:
 * v2.21 — TEMA REVİZE (kullanıcı feedback'i):
 *   - HEADER 4 KUTU: artık üst satır altında TAM EŞİT 4 sütunlu grid (simetrik)
 *   - ARAÇ KARTI: amatör görünen büyük araç ikonu KALDIRILDI;
 *     banner artık 2 kolon — SOL: marka/model/yıl, SAĞ: ARAÇ SAHİBİ + TC
 *     (ayrı "Araç Sahibi" section'ı kaldırıldı — banner içinde)
 *     Logo opsiyonel olarak sağ üst köşede minik gösterilir, yüklenmezse görünmez
 *   - INFOROW yazıları KONTRASTLI: label textSec (eski textMuted yerine),
 *     font weight 700+, font size 10.5/11.5, letter-spacing 0.3
 *   - HEADER butonları üst satırda (DÜZENLE/PORTAL/SİL + aşama dropdown tek sıra)
 *   - Karşı araçta "FARKLI SÜRÜCÜ" section'ı korundu (önemli bilgi)
 *
 * v2.20 — TEMA YENİLEME (alan listesi/sekme yapısı korundu):
...
MR._DosyaDetayInfoRow = ({label, value, mono, bold, color}) => {
  ...
  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:'1px solid ' + C.border}}>
      <span style={{fontSize:10.5,color:C.textSec,fontWeight:700,letterSpacing:0.3,minWidth:90,textTransform:'uppercase'}}>{label}</span>
      <span style={{fontSize:11.5,fontWeight: bold ? 800 : 700, color: color || C.text, fontFamily: mono ? 'monospace' : 'inherit',textAlign:'right',flex:1,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginLeft:8}}>{value || '-'}</span>
    </div>
  );
};
...
/* ═══════════════════════════════════════════════════════════════════════
 * v2.22: MARKA MODEL GÖRSEL EŞLEŞTİRME MODÜLÜ
 * Lisanslı araç görsel veri sağlayıcısı: imagin.studio
 * URL format: cdn.imagin.studio/getimage?customer={KEY}&make=...&modelFamily=...&modelYear=...&angle=01
 *
 * UYARI: Üretim için imagin.studio'dan lisanslı API key alınmalı.
 * Şu an public demo key ile çalışıyor; ticari kullanımda hak ihlali olur.
 * Üretim key alındığında IMAGIN_API_KEY sabitini değiştirmek yeterli.
 *
 * CACHE: localStorage'da 'mr_arac_gorsel_v1' anahtarı altında
 *   { "MARKA_MODEL_YIL": "ok" | "fail" }
 * formatında saklanır. "fail" durumunda URL bile denenmez (fallback gösterilir).
 * "ok" durumunda browser HTTP cache zaten görüntüyü tutar.
 * ═══════════════════════════════════════════════════════════════════════ */
const IMAGIN_API_KEY = 'hrjavascript-mastery'; // TODO: Üretim için lisanslı key ile değiştirin
const IMAGIN_CACHE_KEY = 'mr_arac_gorsel_v1';

const aracGorselUrl = (marka, model, yil) => {
  if (!marka) return null;
  const make = String(marka).toLowerCase().trim().replace(/\s+/g,'');
  const modelFam = String(model || '').toLowerCase().trim().split(/\s+/)[0].replace(/[^a-z0-9]/g,'');
  if (!modelFam) return null;
  const params = new URLSearchParams({
    customer: IMAGIN_API_KEY,
    make: make,
    modelFamily: modelFam,
    angle: '01',
    zoomType: 'fullscreen',
    fileType: 'webp'
  });
  if (yil) params.append('modelYear', String(yil));
  return 'https://cdn.imagin.studio/getimage?' + params.toString();
};

const getGorselCacheStatus = (cacheKey) => {
  try {
    const c = JSON.parse(localStorage.getItem(IMAGIN_CACHE_KEY) || '{}');
    return c[cacheKey] || null;
  } catch { return null; }
};
const setGorselCacheStatus = (cacheKey, status) => {
  try {
    const c = JSON.parse(localStorage.getItem(IMAGIN_CACHE_KEY) || '{}');
    if (c[cacheKey] !== status) {
      c[cacheKey] = status;
      localStorage.setItem(IMAGIN_CACHE_KEY, JSON.stringify(c));
    }
  } catch {}
};

/* v2.22: Araç kartı — ÜSTTE PROFESYONEL ARAÇ GÖRSELİ + ALT BANNER (marka/model/yıl + araç sahibi)
 * Görsel imagin.studio'dan otomatik gelir. Yüklenmezse boş alan değil, sade gradient + tipografi gösterilir. */
const AracGorsel = ({arac, anaRenk}) => {
  const C = MR.C;
  const cacheKey = `${(arac.marka||'').toUpperCase()}_${(arac.model||'').toUpperCase().split(/\s+/)[0]}_${arac.model_yili||''}`;
  const initialStatus = getGorselCacheStatus(cacheKey);
  const [imgError, setImgError] = React.useState(initialStatus === 'fail');
  const [imgLoaded, setImgLoaded] = React.useState(initialStatus === 'ok');
  const aracUrl = aracGorselUrl(arac.marka, arac.model, arac.model_yili);
  const logoUrl = arac.marka ? aracLogoUrl(arac.marka) : null;
  const [logoErr, setLogoErr] = React.useState(false);
  const sahipVar = arac.ruhsat_sahibi || arac.tc_kimlik;

  const onImgLoad = React.useCallback(() => {
    setImgLoaded(true);
    setGorselCacheStatus(cacheKey, 'ok');
  }, [cacheKey]);
  const onImgError = React.useCallback(() => {
    setImgError(true);
    setGorselCacheStatus(cacheKey, 'fail');
  }, [cacheKey]);

  return (
    <div style={{
      borderRadius:10,
      border:`1px solid ${anaRenk}33`,
      overflow:'hidden',
      boxShadow:`0 1px 4px ${anaRenk}10`,
      background: C.bgCard
    }}>
      {/* ═══ ÜST: PROFESYONEL ARAÇ GÖRSELİ ═══ */}
      <div style={{
        height:160,
        background: `linear-gradient(135deg, ${anaRenk}10 0%, ${anaRenk}1f 50%, ${anaRenk}08 100%)`,
        display:'flex', alignItems:'center', justifyContent:'center',
        position:'relative', overflow:'hidden'
      }}>
        {aracUrl && !imgError && (
          <img src={aracUrl} alt={`${arac.marka} ${arac.model}`}
        ...
        {/* Yüklenmedi/başarısız fallback — sade tipografi (boş kalmasın) */}
        {(!aracUrl || imgError || !imgLoaded) && (
          <div style={{textAlign:'center', padding:'0 16px', position:imgLoaded?'absolute':'relative'}}>
            <div style={{fontSize:32,fontWeight:900,color:anaRenk,letterSpacing:1,textTransform:'uppercase',lineHeight:1}}>
              {arac.marka || '—'}
            </div>
            <div style={{fontSize:14,fontWeight:700,color:C.text,marginTop:6,textTransform:'uppercase',letterSpacing:0.3}}>
              {arac.model || ''}
            </div>
            {arac.model_yili && (
              <div style={{fontSize:11,color:C.textSec,marginTop:6,fontWeight:700,fontFamily:'monospace'}}>
                MODEL YILI · {arac.model_yili}
              </div>
            )}
          </div>
        )}
        {/* Marka logosu sağ alt köşede (yüklenmezse gizli) */}
        {logoUrl && !logoErr && imgLoaded && (
          <img src={logoUrl} alt="" onError={() => setLogoErr(true)}
            style={{position:'absolute',bottom:8,right:10,width:34,height:34,objectFit:'contain',opacity:0.9,
              background:'rgba(255,255,255,0.85)',borderRadius:6,padding:3}}/>
        )}
      </div>

      {/* ═══ ALT: MARKA/MODEL/YIL + ARAÇ SAHİBİ İKİ KOLON ═══ */}
      <div style={{
        padding:'12px 14px',
        background: `linear-gradient(180deg, ${anaRenk}08 0%, transparent 100%)`,
        borderTop:`1px solid ${anaRenk}22`
      }}>
        <div style={{display:'grid', gridTemplateColumns: sahipVar ? '1.05fr 1px 1fr' : '1fr', gap:12, alignItems:'stretch'}}>
          {/* SOL: ARAÇ */}
          <div style={{minWidth:0}}>
            <div style={{fontSize:9,fontWeight:800,color:anaRenk,letterSpacing:0.7,marginBottom:3,opacity:0.85}}>ARAÇ</div>
            <div style={{fontSize:14,fontWeight:900,color:anaRenk,letterSpacing:0.3,textTransform:'uppercase',lineHeight:1.2,overflow:'hidden',textOverflow:'ellipsis'}}>
              {arac.marka || '—'}
            </div>
            <div style={{fontSize:11.5,fontWeight:700,color:C.text,marginTop:2,textTransform:'uppercase',lineHeight:1.25,letterSpacing:0.2}}>
              {arac.model || '—'}
            </div>
            <div style={{fontSize:10.5,color:C.textSec,marginTop:3,fontWeight:700,fontFamily:'monospace'}}>
              {arac.model_yili || '—'}
            </div>
          </div>
          {sahipVar && <div style={{background:`linear-gradient(180deg, transparent 0%, ${anaRenk}55 50%, transparent 100%)`}}/>}
          {/* SAĞ: ARAÇ SAHİBİ */}
          {sahipVar && (
            <div style={{minWidth:0}}>
              <div style={{fontSize:9,fontWeight:800,color:anaRenk,letterSpacing:0.7,marginBottom:3,opacity:0.85}}>ARAÇ SAHİBİ</div>
              <div style={{fontSize:12,fontWeight:800,color:C.text,textTransform:'uppercase',lineHeight:1.25,letterSpacing:0.2,marginTop:2,overflow:'hidden',textOverflow:'ellipsis'}}>
                {arac.ruhsat_sahibi || '—'}
              </div>
              {arac.tc_kimlik && (
                <div style={{fontSize:10.5,color:C.textSec,marginTop:6,fontWeight:700,fontFamily:'monospace'}}>
                  T.C. · {arac.tc_kimlik}
                </div>
              )}
            </div>
...
MR.DosyaDetayPage = ({dosyaId, setPage, user}) => {
  ...
  return (
    <div className="fade-in" style={{
      /* v2.22: FULL-BLEED — sayfa container max-width sınırını aş ve tüm pencere genişliğini kullan */
      ...
      {/* v2.21: DOSYA HEADER — 2 satır; alt satırda 4 kutu eşit ve simetrik dağıtılmış */}
      <div style={{...S.card, marginBottom:12, border:`2px solid ${ac}33`, boxShadow:`0 4px 12px ${C.accent}10`}}>
        {/* ÜST SATIR: dosya no + müşteri/sigorta + butonlar */}
        <div style={{padding:'16px 22px 12px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:14}}>
          <div style={{display:'flex', alignItems:'center', gap:14, flex:'1 1 auto', minWidth:280}}>
            ...
            <div>
              ...
              <div style={{fontSize:13, color:C.textSec, marginTop:4, fontWeight:700}}>
                <b style={{color:C.text}}>{magdur.ad_soyad || '—'}</b>
                {dosya.sigorta_sirket && <span> • {dosya.sigorta_sirket}</span>}
                {dosya.hasar_no && <span style={{fontFamily:'monospace',color:C.textSec}}> • {dosya.hasar_no}</span>}
              </div>
            </div>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            {hasYetki('dosya','dosya-duzenle') && <button style={{...S.btn,...S.btnP,fontSize:10,padding:'8px 14px',boxShadow:`0 2px 6px ${C.accent}33`}} onClick={openEditModal}>
              <LIcon name="Edit2" size={12} color="#fff"/> DÜZENLE
            </button>}
            {hasYetki('dosya','dosya-portal') && <button style={{...S.btn,...S.btnS,fontSize:10,padding:'8px 14px',boxShadow:`0 2px 6px ${C.success}33`}} disabled={portalCreating}
            ...
            {hasYetki('dosya','dosya-sil') && <button style={{...S.btn,...S.btnD,fontSize:10,padding:'8px 14px',boxShadow:`0 2px 6px ${C.danger}33`}}
            ...
            <select value={dosya.asama} onChange={e => asamaDegistirOnay(e.target.value)}
              style={{...S.select,width:200,maxWidth:200,fontSize:10,padding:'7px 10px',background:`${ac}11`,border:`1px solid ${ac}33`,color:ac,fontWeight:700}}>
              {ASAMALAR.map(a => <option key={a} value={a}>{a}</option>)}
            </select>
          </div>
        </div>

        {/* ALT SATIR: 4 KUTU TAM EŞİT GRİD (simetrik) */}
        <div style={{padding:'0 22px 16px', display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12}}>
          {[
            ...
            <div key={i} style={{
              ...
              background:MR.tema==='koyu' ? `${b.c}12` : `${b.c}0c`,
              border:`1px solid ${b.c}33`,
              display:'flex', alignItems:'center', gap:11
            }}>
              <div style={{width:34, height:34, borderRadius:8, background:`${b.c}22`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0}}>
                <LIcon name={b.ic} size={16} color={b.c}/>
              </div>
              <div style={{minWidth:0,flex:1}}>
                <div style={{fontSize:9, fontWeight:800, color:C.textSec, letterSpacing:0.6}}>{b.l}</div>
                <div style={{fontSize:12.5, fontWeight:900, color:b.c, marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{b.v}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'bilgi' && (() => {
        ...
        /* v2.22: minmax ile her sütun en az 320px, üstüne tam eşit pay */
        const gridCols = showKarsiArac ? 'minmax(320px, 1fr) minmax(360px, 1fr) minmax(320px, 1fr)' : (showMagdurArac ? 'minmax(320px, 1fr) minmax(360px, 1fr)' : '1fr');
        return (
          ...
          {showMagdurArac && (
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{...S.card, borderTop:`3px solid ${C.success}`, boxShadow:`0 2px 10px ${C.success}10`}}>
                ...
                {/* v2.21: ARAÇ SAHİBİ artık AracGorsel banner'ı içinde — ayrı section kaldırıldı */}
              </div>
          ...
          {showKarsiArac && (
            <div style={{...S.card, borderTop:`3px solid ${C.danger}`, boxShadow:`0 2px 10px ${C.danger}10`}}>
              ...
              {karsiArac.ruhsat_sahibi_surucu === 'farkli' && (karsiArac.surucu_ad || karsiArac.surucu_tc_kimlik) && (
                <div style={{padding:'10px 14px 14px',borderTop:`1px dashed ${C.border}`,background:`${C.warning}10`}}>
                  <div style={{fontSize:9,fontWeight:800,color:C.warning,letterSpacing:0.6,marginBottom:6}}>SÜRÜCÜ (FARKLI KİŞİ)</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 16px'}}>
