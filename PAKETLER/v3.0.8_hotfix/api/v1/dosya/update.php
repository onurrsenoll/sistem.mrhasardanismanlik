...
try {
    ...
    // v3.0.8 — kasko detayları (defansif — varsa atla)
    $db->exec("ALTER TABLE araclar ADD COLUMN IF NOT EXISTS kasko_sirket VARCHAR(100) DEFAULT NULL");
    $db->exec("ALTER TABLE araclar ADD COLUMN IF NOT EXISTS kasko_police VARCHAR(100) DEFAULT NULL");
} catch (\Exception $e) {}
...
try {
    ...
    // Dosya güncelle (v3.0: eksper_paydas_id eklendi)
    $dosyaFields = ['asama', 'dosya_turu', 'talep_turu', 'sigorta_sirket', 'police_no', 'sigorta_turu',
        'dosya_kaynagi', 'avukat_id', 'ortak_id', 'sorumlu_id', 'paydas_id', 'eksper_paydas_id',
        'haklilik', 'komisyon_orani',
    ...
    foreach ($dosyaFields as $field) {
        if (array_key_exists($field, $body)) {
            $sets[] = "$field = ?";
            if (in_array($field, ['avukat_id', 'ortak_id', 'sorumlu_id', 'paydas_id', 'eksper_paydas_id'])) {
    ...
    // Mağdur güncelle (v3.0: gelir_durumu_tipi)
    ...
    // v3.0 — gelir_durumu_tipi='asgari' ise gelir_tutari otomatik asgari ücret olur
    if (isset($body['magdur_gelir_durumu_tipi'])) {
        $tipi = $body['magdur_gelir_durumu_tipi'];
        if ($tipi === 'asgari' && function_exists('mr_get_asgari_ucret')) {
            $body['magdur_gelir_tutari'] = mr_get_asgari_ucret();
        } elseif ($tipi === 'beyan_yok') {
            $body['magdur_gelir_tutari'] = null;
        }
    }
    ...
    foreach ($magdurFields as $field) {
        ...
        if (array_key_exists($key, $body)) {
            ...
            if ($field === 'gelir_tutari') {
                ...
                $mParams[] = !empty($body[$key]) ? $body[$key] : null;
            } elseif ($field === 'gelir_durumu_tipi') {
                $val = $body[$key];
                $mParams[] = in_array($val, ['asgari','asgari_uzeri','beyan_yok'], true) ? $val : 'asgari';
            } else {
    ...
    if ($hasMagdurData && !empty($mSets)) {
        // v3.0 BUG D FIX: Mağdur kaydı var mı kontrol et, yoksa INSERT yap
        $stmtMC = $db->prepare('SELECT id FROM magdurlar WHERE dosya_id = ?');
        $stmtMC->execute([$id]);
        $mevcutMagdur = $stmtMC->fetch();
        ...
        if ($mevcutMagdur) {
            ...
            // INSERT — mağdur kaydı yoktu, oluştur
            ...
            foreach ($magdurFields as $field) {
                ...
                if (array_key_exists($key, $body)) {
                    ...
                    $insertValues[] = '?';
                    if ($field === 'gelir_tutari') {
                        $insertParams[] = !empty($body[$key]) ? (float)$body[$key] : null;
                    } elseif ($field === 'dogum_tarihi') {
                        $insertParams[] = !empty($body[$key]) ? $body[$key] : null;
                    } else {
                        $insertParams[] = !empty($body[$key]) ? clean($body[$key]) : null;
                    }
                }
            }
            $stmt = $db->prepare('INSERT INTO magdurlar (' . implode(', ', $insertFields) . ') VALUES (' . implode(', ', $insertValues) . ')');
    ...
    $magdurAracFields = ['plaka','ruhsat_sahibi','tc_kimlik','marka','model','model_yili','belge_tescil_no','onarim_gun_suresi','gecmis_hasar','kasko','kasko_sirket','kasko_police'];
    $maPrefix = 'ma_';
    $maSets = [];
    $maParams = [];
    $hasMagdurArac = false;
    ...
    // Karşı araç güncelle
    $karsiAracFields = ['plaka','ruhsat_sahibi','tc_kimlik','marka','model','model_yili','belge_tescil_no','trafik_sirket','trafik_police','kasko','kasko_sirket','kasko_police','ruhsat_sahibi_surucu','surucu_ad','surucu_tc_kimlik'];
    $kaPrefix = 'ka_';
    $kaSets = [];
    $kaParams = [];
    $hasKarsiArac = false;
