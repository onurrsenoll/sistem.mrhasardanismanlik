...
MR.DosyaYeniPage = ({setPage, user}) => {
  ...
  const [form, setForm] = useState({
    // Mağdur bilgileri (v3.0: telefon2/email kaldırıldı, gelir_durumu_tipi+gelir_tutari eklendi)
    ad_soyad:'', tc_kimlik:'', telefon:'', iban:'', adres:'', il:'', ilce:'', dogum_tarihi:'',
    cinsiyet:'', gelir_durumu_tipi:'asgari', gelir_tutari:'',
    // Dosya bilgileri
    ...
    // Sigorta bilgileri (v3.0.8: sigorta_turu eklendi)
    sigorta_sirket:'', hasar_no:'', police_no:'', sigorta_turu:'', sorumlu_sigorta:'',
    eksper_firma:'', eksper_paydas_id:'', onarim_servisi:'',
    // ADK araç bilgileri - mağdur (v3.0.8: kasko_sirket, kasko_police eklendi)
    ma_plaka:'', ma_marka:'', ma_model:'', ma_yil:'',
    ma_ruhsat:'', ma_tc:'', ma_belge_tescil:'', ma_onarim_gun:'', ma_gecmis_hasar:'',
    ma_kasko:'', ma_kasko_sirket:'', ma_kasko_police:'',
    // ADK araç bilgileri - karşı (v3.0.8: kasko_sirket, kasko_police eklendi)
    ka_plaka:'', ka_marka:'', ka_yil:'', ka_trafik:'', ka_trafik_police:'',
    ka_ruhsat:'', ka_tc:'', ka_model:'', ka_belge_tescil:'',
    ka_kasko:'', ka_kasko_sirket:'', ka_kasko_police:'',
    ka_ruhsat_surucu:'', ka_surucu_ad:'', ka_surucu_tc:'',
  ...
  // v3.0.8 — Dosya türü değişince talep_turu default ata
  useEffect(() => {
    if (form.dosya_turu === 'ADK' && !form.talep_turu) u('talep_turu', 'DEĞER KAYBI');
    if (form.dosya_turu === 'BH' && !form.talep_turu) u('talep_turu', 'BEDENİ HASAR');
    if (form.dosya_turu === 'MDK' && !form.talep_turu) u('talep_turu', 'MADDİ HASAR');
  }, [form.dosya_turu]);
  ...
  // v3.0 Frontend validasyon — TC algoritma + plaka regex + tarih + IBAN + komisyon
  const validateTC = (tc) => {
    const t = (tc || '').replace(/\D/g, '');
    if (t.length !== 11 || t[0] === '0') return false;
    const odd = +t[0]+ +t[2]+ +t[4]+ +t[6]+ +t[8];
    const even = +t[1]+ +t[3]+ +t[5]+ +t[7];
    const s = odd*7 - even;
    if (s < 0) return false;
    if ((s % 10) != t[9]) return false;
    let sum10 = 0; for (let i=0;i<10;i++) sum10 += +t[i];
    return (sum10 % 10) == t[10];
  };
  const validatePlaka = (p) => /^[0-9]{2}[A-Z]{1,3}[0-9]{2,4}$/.test((p||'').replace(/\s+/g,'').toUpperCase());
  const validateIBAN = (i) => { const x=(i||'').replace(/\s+/g,'').toUpperCase(); return x.length === 26 && /^TR[0-9]{24}$/.test(x); };
  const validateGelecekTarih = (t) => { if (!t) return true; const d = new Date(t); return d <= new Date(); };

  const validate = () => {
    if (!form.ad_soyad.trim()) return 'MAĞDUR ADI SOYADI GEREKLİ';
    if (!form.tc_kimlik.trim()) return 'T.C. KİMLİK NO GEREKLİ';
    if (!validateTC(form.tc_kimlik)) return 'GEÇERSİZ T.C. KİMLİK NO (algoritma hatası)';
    if (!form.telefon.trim()) return 'TELEFON GEREKLİ';
    if (form.iban && !validateIBAN(form.iban)) return 'GEÇERSİZ IBAN (TR + 24 hane olmalı)';
    ...
    if (!validateGelecekTarih(form.kaza_tarihi)) return 'KAZA TARİHİ GELECEK OLAMAZ';
    if (!form.komisyon) return 'KOMİSYON ORANI GEREKLİ';
    const k = parseFloat(form.komisyon);
    if (isNaN(k) || k < 0 || k > 100) return 'KOMİSYON ORANI 0-100 ARASI OLMALI';
    ...
    if (form.ma_plaka && !validatePlaka(form.ma_plaka)) return 'GEÇERSİZ MAĞDUR ARACI PLAKASI (örn: 06ABC123)';
    if (form.ka_plaka && !validatePlaka(form.ka_plaka)) return 'GEÇERSİZ KARŞI ARAÇ PLAKASI (örn: 06ABC123)';
    return null;
  };
  ...
  const kaydet = async () => {
    ...
    // v3.0 — eksper paydaş
    if (gonder.eksper_paydas_id) gonder.eksper_paydas_id = parseInt(gonder.eksper_paydas_id);
    else delete gonder.eksper_paydas_id;
    if (gonder.noter_vekalet) gonder.noter_vekalet = parseFloat(gonder.noter_vekalet);
    // v3.0 — gelir tutarı sayısal
    if (gonder.gelir_tutari) gonder.gelir_tutari = parseFloat(gonder.gelir_tutari);
    else delete gonder.gelir_tutari;
    ...
    <div style={{maxWidth:900,margin:'0 auto',paddingBottom:80}} className="fade-in">
      ...
      <SecCard icon="User" title="MAĞDUR BİLGİLERİ" sub="Mağdur kişiye ait bilgiler" color="#3b82f6" badge="ZORUNLU">
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
          ...
          <FormGroup label="IBAN"><div>
            {(() => {
              const digits = (form.iban||'').replace(/^TR/i,'').replace(/\D/g,'').slice(0,24);
              const formatted = digits.replace(/(.{4})/g,'$1 ').trim();
              const tam = digits.length === 24;
              return <>
                <div style={{position:'relative'}}>
                  <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontWeight:800,color:C.accent,fontSize:13,fontFamily:'monospace',pointerEvents:'none'}}>TR</span>
                  <input
                    style={{...S.input,paddingLeft:34,fontFamily:'monospace',letterSpacing:1,
                      borderColor: digits.length > 0 ? (tam ? C.success : C.warning) : undefined}}
                    value={formatted}
                    onChange={e=>{
                      const yeni = e.target.value.replace(/\D/g,'').slice(0,24);
                      u('iban', yeni ? 'TR' + yeni : '');
                    }}
                ...
                {digits.length > 0 && <div style={{fontSize:9,marginTop:3,color: tam ? C.success : C.warning,fontWeight:600}}>
                  {tam ? '✓ GEÇERLİ FORMAT (24/24)' : `⚠ ${digits.length}/24 HANE — TAMAMLAYIN`}
                </div>}
              </>;
            })()}
          </div></FormGroup>
        ...
        {/* v3.0 — GELİR DURUMU (BH için zorunlu) */}
        {isBH && (
          <div style={{marginTop:8}}>
            <div style={{fontSize:9,fontWeight:700,color:C.textMuted,marginBottom:4}}>GELİR DURUMU *</div>
            <div style={{display:'flex',gap:6,marginBottom:6}}>
              {[
                {v:'asgari', l:'ASGARİ ÜCRET', d:'Yıllık ortalama'},
                {v:'asgari_uzeri', l:'ASGARİ ÜZERİ', d:'Manuel tutar'},
                {v:'beyan_yok', l:'BEYAN YOK', d:'Belirsiz / belge yok'}
              ].map(opt => (
                <div key={opt.v} onClick={() => u('gelir_durumu_tipi', opt.v)} style={{flex:1,padding:8,borderRadius:8,cursor:'pointer',
                  background:form.gelir_durumu_tipi===opt.v?`${C.accent}18`:'transparent',
                  border:`1.5px solid ${form.gelir_durumu_tipi===opt.v?C.accent:C.border}`,textAlign:'center'}}>
                  <div style={{fontSize:11,fontWeight:800,color:form.gelir_durumu_tipi===opt.v?C.accent:C.text}}>{opt.l}</div>
                  <div style={{fontSize:8,color:C.textMuted}}>{opt.d}</div>
                </div>
              ))}
            </div>
            {form.gelir_durumu_tipi === 'asgari_uzeri' && (
              <FormGroup label="GELİR TUTARI (TL/AY) *">
                <input type="number" style={S.input} value={form.gelir_tutari}
                       onChange={e=>u('gelir_tutari',e.target.value)} placeholder="Aylık net tutar girin"/>
              </FormGroup>
            )}
            {form.gelir_durumu_tipi === 'asgari' && (
              <div style={{padding:'6px 10px',background:`${C.success}11`,borderRadius:6,fontSize:10,color:C.textSec}}>
                <LIcon name="Info" size={11}/> {new Date().getFullYear()} NET ASGARİ ÜCRET OTOMATİK ATANACAK
              </div>
            )}
          </div>
        )}
      ...
      <SecCard icon="FileText" title="DOSYA BİLGİLERİ" sub="Tür, tarih, sorumluluk ve sigorta" color="#f97316" badge="ZORUNLU">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
          <FormGroup label="KAZA TARİHİ *"><DateInput value={form.kaza_tarihi} max={new Date().toISOString().split('T')[0]} onChange={v=>{
            if (v) { const d = new Date(v); const today = new Date(); today.setHours(23,59,59,999); if (d > today) { setError('KAZA TARİHİ GELECEK OLAMAZ'); return; } }
            setError(''); u('kaza_tarihi', v);
          }}/></FormGroup>
          <FormGroup label="KAZA İLİ"><select style={S.select} value={form.kaza_il} onChange={e=>{u('kaza_il',e.target.value); u('kaza_ilce','');}}><option value="">SEÇİNİZ</option>{ILLER.map(i=><option key={i} value={i}>{i}</option>)}</select></FormGroup>
          <FormGroup label="KAZA İLÇE"><select style={S.select} value={form.kaza_ilce} onChange={e=>u('kaza_ilce',e.target.value)} disabled={!form.kaza_il}><option value="">{form.kaza_il?'SEÇİNİZ':'ÖNCE İL SEÇİN'}</option>{(ILCELER[form.kaza_il]||[]).map(i=><option key={i} value={i}>{i}</option>)}</select></FormGroup>
      ...
      <SecCard icon="Shield" title="SİGORTA BİLGİLERİ" sub="Sigorta ve hasar dosya bilgileri" color="#10b981" badge="ZORUNLU">
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          ...
          <FormGroup label="SİGORTA TÜRÜ">
            <select style={S.select} value={form.sigorta_turu||''} onChange={e=>u('sigorta_turu',e.target.value)}>
              <option value="">SEÇİNİZ</option><option value="TRAFİK">TRAFİK</option><option value="KASKO">KASKO</option><option value="İMM">İMM</option><option value="ZORUNLU">ZORUNLU</option><option value="İHTİYARİ">İHTİYARİ</option>
            </select>
          </FormGroup>
          ...
          <FormGroup label="DOSYA TALEP TÜRÜ">
            <select style={S.select} value={form.talep_turu} onChange={e=>u('talep_turu',e.target.value)}>
              <option value="">SEÇİNİZ</option><option value="DEĞER KAYBI">DEĞER KAYBI</option><option value="BEDENİ HASAR">BEDENİ HASAR</option><option value="MOTOR DEĞER KAYBI">MOTOR DEĞER KAYBI</option><option value="MADDİ HASAR">MADDİ HASAR</option><option value="DİĞER">DİĞER</option>
            </select>
          </FormGroup>
          ...
          {isADK && <>
            {/* v3.0 — Eksper Paydaş seçimi (paydaslar.tur='eksper') */}
            <FormGroup label="EKSPER FİRMASI (PAYDAŞ)">
              <select style={S.select} value={form.eksper_paydas_id||''} onChange={e=>{
                const id = e.target.value;
                u('eksper_paydas_id', id);
            ...
            <FormGroup label="EKSPER FİRMA ADI (SERBEST)">
              <input style={S.input} value={form.eksper_firma||''} onChange={e=>u('eksper_firma',e.target.value)}
                     placeholder="Paydaş listesinde yoksa manuel"/>
            </FormGroup>
          ...
          {/* v3.0.8: police_no/sorumlu_sigorta artık genel (yukarı taşındı) */}
        </div>
      </SecCard>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div style={{border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden'}}>
              ...
              <div style={{padding:10,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                {String(form.ma_kasko)==='1' && <>
                  <FormGroup label="KASKO ŞİRKETİ"><select style={S.select} value={form.ma_kasko_sirket||''} onChange={e=>u('ma_kasko_sirket',e.target.value)}><option value="">SEÇİNİZ</option>{SIGORTA.map(s=><option key={s} value={s}>{s}</option>)}</select></FormGroup>
                  <FormGroup label="KASKO POLİÇE NO"><input style={S.input} value={form.ma_kasko_police||''} onChange={e=>u('ma_kasko_police',e.target.value)}/></FormGroup>
                </>}
            ...
            <div style={{border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden'}}>
              ...
              <div style={{padding:10,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                {String(form.ka_kasko)==='1' && <>
                  <FormGroup label="KASKO ŞİRKETİ"><select style={S.select} value={form.ka_kasko_sirket||''} onChange={e=>u('ka_kasko_sirket',e.target.value)}><option value="">SEÇİNİZ</option>{SIGORTA.map(s=><option key={s} value={s}>{s}</option>)}</select></FormGroup>
                  <FormGroup label="KASKO POLİÇE NO"><input style={S.input} value={form.ka_kasko_police||''} onChange={e=>u('ka_kasko_police',e.target.value)}/></FormGroup>
                </>}
