...
MR.MuhasebePage = ({setPage, user, subPage}) => {
  ...
  const sekmeler = [
    ...
    {key:'mutabakat',label:'AYLIK MUTABAKAT',       icon:'FileSpreadsheet'}
  ];
...
/* ═══════════════════════════════════════════════════════════
   SEKME 7 – AYLIK MUTABAKAT (v3.0 — Sayfa içi canlı rapor + temiz PDF)
   ═══════════════════════════════════════════════════════════ */
const AylikMutabakat = ({setPage, user}) => {
  ...
  const [dosyalar, setDosyalar] = useState([]); // editable list
  ...
  const [pozToplam, setPozToplam] = useState(0);
  const [mahsupAdet, setMahsupAdet] = useState(0);
  const [mahsupToplam, setMahsupToplam] = useState(0);

  const yukle = async () => {
    setLoading(true);
    try {
      const tok = localStorage.getItem('mr_token') || localStorage.getItem('token') || localStorage.getItem('jwt') || '';
      const r = await fetch(`/api/v1/muhasebe/aylik-mutabakat-data.php?donem=${yil}-${ay}`, {
      ...
      if (j.success) {
        ...
        setPozToplam(j.data.kapanis?.poz_toplam || 0);
        setMahsupAdet(j.data.kapanis?.mahsup_adet || 0);
        setMahsupToplam(j.data.kapanis?.mahsup_toplam || 0);
      }
    } catch (e) {}
    setLoading(false);
  };
  ...
  const b1 = dosyaTop - dev - ayi;        // negatif: fazla ödeme; pozitif: borç
  ...
  const updateDosya = (idx, key, val) => {
    setDosyalar(p => p.map((d,i) => i === idx ? {...d, [key]: val} : d));
  };
  ...
  // ═══ Print — sayfayı izole et, sadece raporu yazdır ═══
  const yazdir = () => {
  ...
  // ═══ Rapor stili (inline — print için izole) ═══
  const raporStyles = `
  ...
  const FIRMA = data?.firma_adi || 'MR HASAR DANIŞMANLIK';
  const HAZIRLAYAN = data?.hazirlayan || (user?.ad_soyad || 'SİSTEM').toUpperCase();
  ...
  return (
    <div className="fade-in">
      ...
      {/* ═══ KONTROL PANELİ ═══ */}
      <div style={{...S.card, marginBottom:18}} data-mutabakat-form>
        <SectionTitle icon="FileSpreadsheet" title="AYLIK MUTABAKAT RAPORU" sub="MEVCUT TEMA · CANLI ÖNİZLEME · TEK BUTON PDF"/>
        <div style={{padding:'16px 20px'}}>

          {/* Dönem + Yenile + Yazdır */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr auto auto',gap:12,alignItems:'flex-end',marginBottom:14}}>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>YIL</label>
              <select value={yil} onChange={e=>setYil(e.target.value)} style={{...S.select,padding:'10px 12px',fontSize:13}}>
                {yillar.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>AY</label>
              <select value={ay} onChange={e=>setAy(e.target.value)} style={{...S.select,padding:'10px 12px',fontSize:13}}>
                {aylar.map(a => <option key={a.v} value={a.v}>{a.l}</option>)}
              </select>
            </div>
            <button onClick={yukle} disabled={loading} style={{...S.btn,...S.btnP,padding:'12px 22px',fontSize:13,fontWeight:800}}>
              <LIcon name="RefreshCw" size={14} color="#fff"/> {loading ? 'YÜKLENİYOR...' : 'VERİYİ ÇEK'}
            </button>
            <button onClick={yazdir} style={{...S.btn,padding:'12px 22px',fontSize:13,fontWeight:800,background:'#27AE60',color:'#fff',border:0}}>
              <LIcon name="Printer" size={14} color="#fff"/> PDF / YAZDIR
            </button>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:10,marginBottom:14}}>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>ÖNCEKİ AY DEVİR (₺)</label>
              <input type="number" value={devir} onChange={e=>setDevir(e.target.value)} placeholder="0" style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>AY İÇİ ÖDEMELER (₺)</label>
              <input type="number" value={ayIci} onChange={e=>setAyIci(e.target.value)} placeholder="0" style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>SBM SORGU (₺)</label>
              <input type="number" value={sbm} onChange={e=>setSbm(e.target.value)} placeholder="0" style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>EKRAN ÖDEME (₺)</label>
              <input type="number" value={ekran} onChange={e=>setEkran(e.target.value)} placeholder="0" style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>ATK MASRAFI (₺)</label>
              <input type="number" value={atk} onChange={e=>setAtk(e.target.value)} placeholder="0" style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
          </div>

          {/* Bölüm 3 manuel override */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:10}}>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>POZİTİF DOSYA ADET</label>
              <input type="number" value={pozAdet} onChange={e=>setPozAdet(e.target.value)} style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>POZİTİF TOPLAM (₺)</label>
              <input type="number" value={pozToplam} onChange={e=>setPozToplam(e.target.value)} style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>MAHSUP ADET</label>
              <input type="number" value={mahsupAdet} onChange={e=>setMahsupAdet(e.target.value)} style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
            <div>
              <label style={{...S.label,marginBottom:4,display:'block'}}>MAHSUP TOPLAM (₺)</label>
              <input type="number" value={mahsupToplam} onChange={e=>setMahsupToplam(e.target.value)} style={{...S.input,padding:'8px 10px',fontSize:12}}/>
            </div>
          </div>
          ...
          <div style={{marginTop:16}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
              <div style={{fontSize:11,fontWeight:700,color:C.textSec,letterSpacing:0.5}}>
                DOSYA LİSTESİ ({dosyalar.length} dosya · sistemden çekildi · düzenlenebilir)
              </div>
              <button onClick={addDosya} style={{...S.btn,padding:'5px 10px',fontSize:11,background:'#3498DB',color:'#fff',border:0,borderRadius:4}}>
                + EKLE
              </button>
            </div>
            <div style={{maxHeight:280,overflow:'auto',border:`1px solid ${C.border}`,borderRadius:6}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                ...
                <tbody>
                  {dosyalar.length === 0 && (
                    <tr><td colSpan={6} style={{padding:18,textAlign:'center',color:C.textMuted,fontStyle:'italic',fontSize:11}}>BU AY KAYIT YOK · "+ EKLE" ile manuel ekleyebilirsin</td></tr>
                  )}
                  {dosyalar.map((d,i) => (
                    <tr key={i} style={{borderBottom:`1px solid ${C.border}`}}>
                      ...
                      <td style={{padding:3}}><input type="number" value={d.tutar||0} onChange={e=>updateDosya(i,'tutar',e.target.value)} style={{width:'100%',padding:'4px 6px',fontSize:11,border:`1px solid ${C.border}`,borderRadius:3,textAlign:'right'}}/></td>
          ...
          <div className="mt-page">
            ...
            <div className="mt-body">
              ...
              <div className="mt-info-box">
                ...
                <p>BU RAPOR, {ayAdi} {yil} AY SONU İTİBARIYLA <strong>{FIRMA} CARİ HESAP MUTABAKATINI</strong> ÇIKARMAK AMACIYLA HAZIRLANMIŞTIR. ÜÇ BÖLÜMDEN OLUŞUR; FİNAL BAKİYE, BÖLÜMLERİN ARİTMETİK TOPLAMINDAN OLUŞAN NET POZİSYONDUR.</p>
                ...
                <tbody>
                  ...
                  {dosyalar.map((d,i) => (
                    <tr key={i}>
                      ...
                      <td>{d.tip}</td>
                      <td>{d.kanal}</td>
                      <td className="right">{fmt(num(d.tutar))}</td>
                    </tr>
                  ))}
              ...
              <table className="mt-table">
                ...
                <tbody>
                  ...
                  <tr><td>{ayAdi} {yil} DOSYA MASRAFLARI TOPLAMI</td><td className="right mt-red">{fmt(dosyaTop)}</td></tr>
                  <tr className="totals"><td>BÖLÜM 1 NET — {b1Pos ? 'TAHSİL EDİLECEK' : 'FAZLA ÖDEME'}</td><td className={'right ' + (b1Pos?'red':'orange')}>{fmt(b1)}</td></tr>
                </tbody>
              </table>
              ...
              {/* BÖLÜM 2 */}
              <h2 className="mt-section-title">⚖️ BÖLÜM 2 — ORTAK MASRAFLAR / {FIRMA} ALACAĞI</h2>
              <p style={{fontSize:12,color:'#7F8C8D',marginBottom:12}}>
                BU MASRAFLAR <strong>{FIRMA} TARAFINDAN ÖDENMİŞTİR</strong>. SBM SORGU VE EKRAN ÖDEME MASRAFLARININ <strong>YARISI</strong>{at>0?', ATK MASRAFININ İSE TAMAMI':''} {FIRMA}'IN ALACAĞI OLARAK RAPORA EKLENİR.
              </p>
              ...
              <table className="mt-table">
                ...
                <tbody>
                  ...
                  {(sb+ek+at)===0 && <tr><td colSpan={2} style={{textAlign:'center',color:'#95A5A6',fontStyle:'italic',padding:10}}>ORTAK MASRAF GİRİLMEDİ</td></tr>}
                  <tr className="totals"><td>{FIRMA}'IN ÖDEDİĞİ TOPLAM</td><td className="right">{fmt(sb+ek+at)}</td></tr>
                </tbody>
              </table>

              <div className="mt-subsection-title">{FIRMA} ALACAĞININ HESAPLANMASI</div>
              <table className="mt-table">
                ...
                <tbody>
                  ...
                  <tr className="totals"><td>TOPLAM {FIRMA} ALACAĞI</td><td className="right">{fmt(sb+ek+at)}</td><td className="right blue">{fmt(b2)}</td></tr>
                </tbody>
              </table>
              ...
              {/* BÖLÜM 3 */}
              <h2 className="mt-section-title">💼 BÖLÜM 3 — {ayAdi} {yil} KAPANAN DOSYALAR NET KAZANCI</h2>
              <p style={{fontSize:12,color:'#7F8C8D',marginBottom:12}}>
                {ayAdi} AYINDA KAPANAN DOSYALARDAN {FIRMA}'IN HAK ETTİĞİ KAZANÇ TUTARI VE MAHSUPLAR.
              </p>

              <table className="mt-table">
                <thead><tr><th>AÇIKLAMA</th><th className="right">DOSYA</th><th className="right">TUTAR</th></tr></thead>
                <tbody>
                  <tr><td>POZİTİF TOPLAM (KAZANÇLAR — MR HAK EDİŞİ)</td><td className="right">{pozAdet}</td><td className="right mt-green">{fmt(num(pozToplam))}</td></tr>
                  <tr><td>MAHSUPLAR (EKSİLER — İADE / KESİNTİ / DÜZELTME)</td><td className="right">{mahsupAdet}</td><td className="right mt-red">−{fmtPlain(num(mahsupToplam))} ₺</td></tr>
                  <tr className="totals"><td>BÖLÜM 3 — KAPANAN DOSYA HAK EDİŞİ</td><td className="right">{num(pozAdet)+num(mahsupAdet)}</td><td className={'right ' + (b3Pos?'green':'red')}>{fmt(b3)}</td></tr>
                </tbody>
              </table>
              ...
              <div className={'mt-result-box ' + (b3Pos ? 'mt-rb-green' : 'mt-rb-red')}>
                ...
                <div className="nt">{b3Pos ? `(${FIRMA}'A TAHSİL EDİLECEK)` : '(NEGATİF — MAHSUPLAR KAZANÇLARI AŞTI)'}</div>
              </div>

              <hr className="mt-divider"/>

              {/* FİNAL */}
              <h2 className="mt-section-title">🧮 FİNAL BAKİYE — {ayAdi} {yil} CARİ HESAP KAPANIŞI</h2>

              <div className="mt-cards">
                <div className={'mt-card ' + (b1Pos ? 'mt-card-red' : 'mt-card-orange')}>
                  <div className="lbl">BÖLÜM 1 {b1Pos ? 'ALACAĞI' : '(FAZLA ÖDEME)'}</div>
                  <div className="val">{fmt(b1)}</div>
                </div>
                <div className="mt-card mt-card-blue">
                  <div className="lbl">BÖLÜM 2 ALACAĞI</div>
                  <div className="val">+{fmtPlain(b2)} ₺</div>
                </div>
                <div className={'mt-card ' + (b3Pos ? 'mt-card-green' : 'mt-card-red')}>
                  <div className="lbl">BÖLÜM 3 {b3Pos ? 'ALACAĞI' : '(NEGATİF)'}</div>
                  <div className="val">{fmt(b3)}</div>
                </div>
              </div>
              ...
              <table className="mt-table mt-final-table">
                ...
                <tbody>
                  <tr className={b1Pos ? 'row-red' : 'row-orange'}><td>BÖLÜM 1 — {b1Pos ? 'DOSYA MASRAFI BAKİYESİ' : 'FAZLA ÖDEME (MR BORCU)'}</td><td>{fmt(b1)}</td></tr>
                  <tr className="row-blue"><td>BÖLÜM 2 — ORTAK MASRAF ALACAĞI</td><td>+{fmtPlain(b2)} ₺</td></tr>
                  <tr className={b3Pos ? 'row-green' : 'row-red'}><td>BÖLÜM 3 — {b3Pos ? 'KAPANAN DOSYA HAK EDİŞİ' : 'KAPANAN DOSYA NET ZARAR'}</td><td>{fmt(b3)}</td></tr>
              ...
              <div className="mt-result-box mt-rb-final">
                ...
                <div className="nt">{fPos ? `${FIRMA}'A KALAN NET ÖDEME TUTARI` : `${FIRMA}'IN NET BORCU (SONRAKİ AYA DEVREDİLECEK)`}</div>
              </div>
            ...
            <div className="mt-footer">
              <strong>{FIRMA}</strong> &nbsp;·&nbsp; {ayAdi} {yil} AY SONU RAPORU &nbsp;·&nbsp; HAZIRLAYAN: {HAZIRLAYAN}
            </div>
          </div>
        </div>
      </div>
