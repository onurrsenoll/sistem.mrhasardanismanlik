/**
 * MR HASAR DANIŞMANLIK — DOSYA DETAY (v2.13)
...
MR.DosyaDetayPage = ({dosyaId, setPage, user}) => {
  ...
  const openEditModal = () => {
    ...
    setEditForm({
      ...
      // Karşı araç alanları (v2.13: ka_plaka eklendi)
  ...
  const dosyaGuncelle = async () => {
    ...
    const updateData = {
      ...
      magdur_email: editForm.magdur_email || '',
    ...
    // Mağdur ve karşı araç alanları (ADK / MDK)
    if (editForm.dosya_turu === 'ADK' || editForm.dosya_turu === 'MDK') {
      // Mağdur araç (v2.13: ma_plaka/marka/model/yil/kasko eklendi)
      ...
      // Karşı araç (v2.13: ka_plaka eklendi)
  ...
  return (
    <div className="fade-in">
      ...
      <Modal open={editM} onClose={() => setEditM(false)} title="DOSYA BİLGİLERİ DÜZENLE — TÜM ALANLAR" width="780px">
        ...
        {/* v2.13: SİGORTA & EKSPER & SERVİS BÖLÜMÜ */}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>
          ...
          <FormGroup label="SİGORTA BRANŞI">
            <select value={editForm.sigorta_brans||''} onChange={e => u('sigorta_brans',e.target.value)} style={{...S.select,padding:'8px 10px',fontSize:11}}>
              <option value="">SEÇİNİZ</option>
              {['KASKO','TRAFİK','DASK','KONUT','İŞYERİ','SAĞLIK','HAYAT','YANGIN','NAKLİYAT','MÜHENDİSLİK','SORUMLULUK','FERDİ KAZA','İMM','TSS','DİĞER'].map(b => <option key={b} value={b}>{b}</option>)}
            </select>
          </FormGroup>
          <FormGroup label="SİGORTA TÜRÜ">
            <input value={editForm.sigorta_turu||''} onChange={e => u('sigorta_turu',e.target.value)} placeholder="ÖRN: TRAFİK / KASKO / İHTİYARİ" style={{...S.input,padding:'8px 10px',fontSize:11}}/>
          </FormGroup>
          <FormGroup label="DOSYA TALEP TÜRÜ">
            <select value={editForm.talep_turu||''} onChange={e => u('talep_turu',e.target.value)} style={{...S.select,padding:'8px 10px',fontSize:11}}>
              <option value="">SEÇİNİZ</option>
              <option value="TAZMİNAT">TAZMİNAT</option>
              <option value="HASAR ÖDEME">HASAR ÖDEME</option>
              <option value="DEĞER KAYBI">DEĞER KAYBI</option>
              <option value="ONARIM SÜRESİ">ONARIM SÜRESİ</option>
              <option value="MAHRUMİYET">MAHRUMİYET</option>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>
          ...
          <FormGroup label="E-POSTA">
            <input type="email" value={editForm.magdur_email||''} onChange={e => u('magdur_email',e.target.value)} placeholder="ornek@mail.com" style={{...S.input,padding:'8px 10px',fontSize:11}}/>
          </FormGroup>
        </div>
        {/* v2.13: ADRES tek satır */}
        <div style={{marginBottom:14}}>
