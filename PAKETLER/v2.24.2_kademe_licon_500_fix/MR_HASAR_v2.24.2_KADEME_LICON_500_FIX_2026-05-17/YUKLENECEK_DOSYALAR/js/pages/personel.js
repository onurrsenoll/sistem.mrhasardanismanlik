/* ============================================================
   MR HASAR DANIŞMANLIK – PERSONEL YÖNETİMİ (personel.js) — v2.24.1
   PERSONEL LİSTESİ, YENİ PERSONEL, HAKEDİŞ TAKİBİ, BİLGİ KARTI,
   KADEMELİ PRİM (PARÇALI), REAL-TIME CARİ, ÖZLÜK EVRAKLARI

   v2.24.2 HOTFIX: KademeEditor LIcon CRASH + cari-ozet 500 defensif
     [SORUN] ÇALIŞMA MODELİ sekmesi tıklayınca console'da
       "ReferenceError: LIcon is not defined" fırlatıyor, modal çöküyor.
       KademeEditor modül-top component (PersonelListesi dışında) —
       LIcon scope'a destructure edilmemiş.
     [ÇÖZÜM] `const LIcon = MR.LIcon;` eklendi.

   v2.24.1 HOTFIX: ÇALIŞMA MODELİ sekmesi boş açılıyordu
     [SORUN] Render koşulu `detayTab === 'model' && detayModelForm`
       şeklindeydi. Modal açılırken detayModelForm yalnız cari-ozet.php
       başarılı dönerse set ediliyordu. Cari boş veya yavaş yanıt
       verince ÇALIŞMA MODELİ sekmesinde içerik HİÇ render edilmiyordu
       (sekmelerin altı bomboş).
     [ÇÖZÜM] detayAc() başında personel kolonlarından default form
       set edilir. Cari döndüğünde gerçek aktif modelle override edilir.
       Sekme her zaman dolu açılır.

   v2.24: KADEMELİ PRİM SİSTEMİ + CARİ ENTEGRASYON + ÖZLÜK EVRAKI
     [SORUN] Dosya sorumlusu atandığında personelin carisine otomatik
       prim yansımıyordu. Çalışma modeli yalnız sabit ₺/dosya idi;
       kademe yoktu. Özlük evrak yükleme arayüzü yoktu.
     [ÇÖZÜM] Personel bilgi kartı 5 sekmeli REAL-TIME modüle döndü:
       1) GENEL BİLGİLER
       2) CARİ HESAP — dönem bazlı, ÖDEME YAP butonlu (atomik kasa düşüş)
       3) PRİM HAKEDİŞ (DÖNEM ACCORDION) — her dönem ADK+BH dosyalar
       4) ÇALIŞMA MODELİ — model tipi seçici (sabit/sabit_prim/
            kademeli_ofis/kademeli_saha) + KademeEditor (min/max/birim)
       5) ÖZLÜK EVRAKLARI — upload (PDF/JPG/PNG/DOCX), indir, sil
     [VERİ AKIŞI]
       · /personel/cari-ozet.php  — REAL-TIME parçalı kademe hesabı
       · /personel/kazanc-model.php (GET/POST) — kişi başına bareme
       · /personel/odeme-yap.php  — kasa hareketi ile atomik ödeme
       · /personel/evrak-yukle.php (multipart), evrak-liste/indir/sil
       · dosya/create.php + update.php → _dosya_prim_hook.php otomatik
         personel_prim_kayitlari'na ekler/iptal eder/aktif eder
     [HESAP] PARÇALI (vergi gibi):
       25 dosya · (0-10:400, 11-25:600) → 10×400 + 15×600 = 13.000
...
/* ═══ DEPARTMANLAR ═══ */
/* v2.24: kademe JSON güvenli parse helper */
const parseJsonSafe = (txt, fallback) => {
  if (!txt) return fallback;
  if (typeof txt === 'object') return txt;
  try { const j = JSON.parse(txt); return j == null ? fallback : j; }
  catch(e) { return fallback; }
};
/* v2.24: standart özlük evrak tipleri (kullanıcı 'STANDART LİSTE + SERBEST' seçti) */
const EVRAK_TIPLERI = [
  {k:'sozlesme',      l:'SÖZLEŞME'},
  {k:'kimlik',        l:'KİMLİK'},
  {k:'sgk_giris',     l:'SGK İŞE GİRİŞ'},
  {k:'diploma',       l:'DİPLOMA'},
  {k:'ehliyet',       l:'EHLİYET'},
  {k:'ikametgah',     l:'İKAMETGAH'},
  {k:'saglik_raporu', l:'SAĞLIK RAPORU'},
  {k:'adli_sicil',    l:'ADLİ SİCİL'},
  {k:'foto',          l:'FOTOĞRAF'},
  {k:'diger',         l:'DİĞER'}
];
...
const PersonelListesi = ({setPage, user}) => {
  ...
  /* v2.24: PERSONEL BİLGİ KARTI MODAL (5 sekme: GENEL / CARİ HESAP / PRİM HAKEDİŞ /
     ÇALIŞMA MODELİ / ÖZLÜK EVRAKLARI). cari-ozet.php REAL-TIME entegre. */
  ...
  const [detayEvraklar, setDetayEvraklar] = useState([]);
  const [detayModelForm, setDetayModelForm] = useState(null);
  const [detayLoading, setDetayLoading] = useState(false);
  const [detayDonemAcik, setDetayDonemAcik] = useState({});
  const [evrakYukleForm, setEvrakYukleForm] = useState({turu:'sozlesme', etiket:'', file:null});
  /* CARİ HESAP içi ÖDEME modal'ı için */
  const [detayOdemeForm, setDetayOdemeForm] = useState(null);
  ...
  const detayAc = async (p) => {
    ...
    setDetayEvraklar([]);
    setDetayDonemAcik({});
    /* v2.24.1: ÇALIŞMA MODELİ sekmesi cari-ozet beklemeden açılabilsin diye
       önce personel kolonlarından default formu hemen kur (cari döndüğünde
       aktif model bilgisiyle override edilecek). */
    setDetayModelForm({
      personel_id: p.id,
      model_tipi: 'sabit_prim',
      sabit_maas: parseFloat(p.maas) || 0,
      prim_adk:   parseFloat(p.prim_adk) || 0,
      prim_bh:    parseFloat(p.prim_bh)  || 0,
      saha_kademe: [],
      ofis_adk_kademe: [],
      ofis_bh_kademe: [],
      notlar: ''
    ...
    try {
      const [c, e] = await Promise.all([
        api.req('/personel/cari-ozet.php?personel_id=' + p.id),
        api.req('/personel/evrak-liste.php?personel_id=' + p.id)
      ]);
      if (c?.success) {
        setDetayCari(c.data);
        const m = c.data?.model;
        const ofisKota = parseJsonSafe(m?.ofis_kota_json, {});
        setDetayModelForm({
          personel_id: p.id,
          model_tipi: m?.model_tipi || 'sabit_prim',
          sabit_maas: parseFloat(m?.sabit_maas) || parseFloat(p.maas) || 0,
          prim_adk:   parseFloat(m?.prim_adk)   || parseFloat(p.prim_adk) || 0,
          prim_bh:    parseFloat(m?.prim_bh)    || parseFloat(p.prim_bh)  || 0,
          saha_kademe: parseJsonSafe(m?.saha_kademe_json, []),
          ofis_adk_kademe: Array.isArray(ofisKota?.adk_kademe) ? ofisKota.adk_kademe : [],
          ofis_bh_kademe:  Array.isArray(ofisKota?.bh_kademe)  ? ofisKota.bh_kademe  : [],
          notlar: m?.notlar || ''
        });
      }
      if (e?.success) setDetayEvraklar(e.data?.evraklar || []);
    } catch(err) {}
    setDetayLoading(false);
  };
  const detayKapat = () => { setDetayPersonel(null); setDetayCari(null); setDetayEvraklar([]); };
  const detayDonemToggle = (idx) => setDetayDonemAcik(s => ({...s, [idx]: !s[idx]}));

  const detayCariYenile = async () => {
    if (!detayPersonel) return;
    try {
      const r = await api.req('/personel/cari-ozet.php?personel_id=' + detayPersonel.id);
      if (r?.success) setDetayCari(r.data);
    } catch(e) {}
  };

  const modelKaydet = async () => {
    if (!detayModelForm) return;
    const f = detayModelForm;
    const body = {
      personel_id: f.personel_id, model_tipi: f.model_tipi,
      sabit_maas: f.sabit_maas, prim_adk: f.prim_adk, prim_bh: f.prim_bh,
      gecerlilik_baslangic: new Date().toISOString().slice(0,10), notlar: f.notlar
    };
    if (f.model_tipi === 'kademeli_saha') {
      body.saha_kademe_json = JSON.stringify(f.saha_kademe || []);
    } else if (f.model_tipi === 'kademeli_ofis') {
      body.ofis_kota_json = JSON.stringify({adk_kademe: f.ofis_adk_kademe || [], bh_kademe: f.ofis_bh_kademe || []});
    }
    try {
      const r = await api.req('/personel/kazanc-model.php', {method:'POST', body: JSON.stringify(body)});
      if (r?.success) {
        setBasari('ÇALIŞMA MODELİ KAYDEDİLDİ'); setTimeout(()=>setBasari(''),3000);
        await detayCariYenile(); await yukle();
      } else {
        setHata(r?.message || 'Model kaydedilemedi'); setTimeout(()=>setHata(''),4000);
      }
    } catch(e) { setHata('Hata: '+e.message); setTimeout(()=>setHata(''),4000); }
  };

  const evrakYukle = async () => {
    if (!detayPersonel || !evrakYukleForm.file) { setHata('DOSYA SEÇİN'); setTimeout(()=>setHata(''),3000); return; }
    const fd = new FormData();
    fd.append('personel_id', detayPersonel.id);
    fd.append('evrak_turu', evrakYukleForm.turu);
    fd.append('evrak_etiketi', evrakYukleForm.etiket || '');
    fd.append('dosya', evrakYukleForm.file);
    try {
      /* Native fetch — api.req FormData desteklemiyor olabilir, doğrudan token ile yolla */
      const tok = (MR.api && MR.api.token) || (window.api && window.api.token) || '';
      const resp = await fetch('/api/v1/personel/evrak-yukle.php', {
        method:'POST', body: fd,
        headers: tok ? {'Authorization':'Bearer ' + tok} : {}
      });
      const r = await resp.json();
      if (r?.success) {
        setBasari('EVRAK YÜKLENDİ'); setTimeout(()=>setBasari(''),3000);
        setEvrakYukleForm({turu:'sozlesme', etiket:'', file:null});
        const e = await api.req('/personel/evrak-liste.php?personel_id=' + detayPersonel.id);
        if (e?.success) setDetayEvraklar(e.data?.evraklar || []);
      } else { setHata(r?.message || 'Yüklenemedi'); setTimeout(()=>setHata(''),4000); }
    } catch(err) { setHata('Hata: '+err.message); setTimeout(()=>setHata(''),4000); }
  };

  const evrakSil = async (id) => {
    if (!window.confirm('Evrak silinsin mi?')) return;
    try {
      const r = await api.req('/personel/evrak-sil.php', {method:'POST', body: JSON.stringify({id})});
      if (r?.success) {
        setDetayEvraklar(list => list.filter(x => x.id !== id));
        setBasari('EVRAK SİLİNDİ'); setTimeout(()=>setBasari(''),3000);
      }
    } catch(e) {}
  };

  /* Modal içi ödeme: cari-ozet.php sonrası odeme-yap.php */
  const detayOdemeAc = (donem, tutar) => {
    if (!detayPersonel) return;
    setDetayOdemeForm({
      personel_id: detayPersonel.id,
      odeme_turu: 'maas',
      tutar: tutar || 0,
      kasa_id: kasalar?.[0]?.id || 0,
      donem: donem || '',
      odeme_tarihi: new Date().toISOString().slice(0,10),
      aciklama: ''
    });
  };
  const detayOdemeKaydet = async () => {
    if (!detayOdemeForm) return;
    try {
      const r = await api.req('/personel/odeme-yap.php', {method:'POST', body: JSON.stringify(detayOdemeForm)});
      if (r?.success) {
        setBasari('ÖDEME KAYDEDİLDİ — KASA DÜŞÜLDÜ'); setTimeout(()=>setBasari(''),3000);
        setDetayOdemeForm(null);
        await detayCariYenile();
      } else { setHata(r?.message || 'Ödeme yapılamadı'); setTimeout(()=>setHata(''),4000); }
    } catch(e) { setHata('Hata: '+e.message); setTimeout(()=>setHata(''),4000); }
  };
  ...
  return (
    <div>
      ...
      {/* ═══════════════════════════════════════════════════════════
          v2.24: PERSONEL BİLGİ KARTI MODAL — 5 sekme + REAL-TIME CARİ
          ═══════════════════════════════════════════════════════════ */}
      <Modal open={!!detayPersonel} onClose={detayKapat}
        ...
        {detayPersonel && (() => {
          ...
          /* v2.24: tüm finansal veriler cari-ozet.php'den REAL-TIME geliyor */
          const cari = detayCari || {donemler:[], genel_hakedis:0, genel_odenen:0, genel_bakiye:0, son_odemeler:[], model:null};
          const aylikMaas = parseFloat(cari.model?.sabit_maas) || parseFloat(p.maas) || 0;
          const genelHakedis = parseFloat(cari.genel_hakedis) || 0;
          const genelOdenen  = parseFloat(cari.genel_odenen)  || 0;
          const genelBakiye  = parseFloat(cari.genel_bakiye)  || 0;
          ...
          const sekmeler = [
            ...
            {key:'evrak',  label:'ÖZLÜK EVRAKLARI',       icon:'Folder'}
          ];
          ...
          return (
            <div>
              ...
              {/* ═══ SEKME 2: CARİ HESAP (REAL-TIME) ═══ */}
              {detayTab === 'cari' && (
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4, 1fr)',gap:10,marginBottom:14}}>
                    {[
                      {ic:'Wallet',     l:'AYLIK SABİT MAAŞ', v:paraBirim(aylikMaas),    c:C.success},
                      {ic:'TrendingUp', l:'TOPLAM HAKEDİŞ',   v:paraBirim(genelHakedis), c:C.cyan},
                      {ic:'CheckCircle',l:'TOPLAM ÖDENEN',    v:paraBirim(genelOdenen),  c:C.accent},
                      {ic:'Clock',      l:'BEKLEYEN BAKİYE',  v:paraBirim(genelBakiye),  c:genelBakiye>0?C.warning:C.textMuted}
                    ].map((b,i) => (
                  ...
                  {/* DÖNEM BAZLI CARİ TABLO */}
                  <div style={{...S.card,marginBottom:12}}>
                    <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,background:`${C.accent}08`,display:'flex',alignItems:'center',gap:8}}>
                      <LIcon name="List" size={14} color={C.accent}/>
                      <span style={{fontSize:11,fontWeight:800,color:C.accent,letterSpacing:0.4}}>DÖNEM BAZLI CARİ HESAP ({cari.donemler.length})</span>
                      <span style={{marginLeft:'auto',fontSize:10,color:C.textMuted}}>REAL-TIME · dosyalardan canlı</span>
                    </div>
                    {detayLoading ? (
                      <div style={{padding:30,textAlign:'center',color:C.textMuted,fontSize:11}}>YÜKLENİYOR...</div>
                    ) : cari.donemler.length === 0 ? (
                      <div style={{padding:30,textAlign:'center',color:C.textMuted,fontSize:11}}>HENÜZ HAREKET YOK</div>
                    ) : (
                      <table style={{width:'100%',fontSize:11,borderCollapse:'collapse'}}>
                        <thead style={{background:`${C.accent}10`}}>
                          <tr>
                            {['DÖNEM','ADK','BH','MAAŞ','PRİM','HAKEDİŞ','ÖDENEN','BAKİYE','İŞLEM'].map(h => (
                        ...
                        <tbody>
                          {cari.donemler.map((d,i) => (
                            <tr key={i} style={{borderBottom:`1px solid ${C.border}66`,background: i%2===0?'transparent':`${C.accent}05`}}>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',fontWeight:800,color:C.text}}>{d.donem}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',color:C.cyan}}>{d.adk_sayisi}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',color:C.purple}}>{d.bh_sayisi}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',color:C.success}}>{paraBirim(d.maas)}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',color:C.cyan}}>{paraBirim(d.toplam_prim)}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',fontWeight:800,color:C.text}}>{paraBirim(d.toplam_hakedis)}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',color:C.accent}}>{paraBirim(d.odenen)}</td>
                              <td style={{padding:'6px 10px',fontFamily:'monospace',fontWeight:800,color:(d.bakiye||0)>0?C.warning:C.success}}>{paraBirim(d.bakiye)}</td>
                              <td style={{padding:'6px 10px'}}>
                                {(d.bakiye||0) > 0 && MR.hasYetki(user,'muhasebe','personel-maas-ode') ? (
                                  <button style={{...S.btn,...S.btnS,fontSize:8,padding:'3px 8px'}}
                                    onClick={() => detayOdemeAc(d.donem, d.bakiye)}>
                  ...
                  {/* SON ÖDEMELER */}
                  <div style={S.card}>
                    <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,background:`${C.success}08`,display:'flex',alignItems:'center',gap:8}}>
                      <LIcon name="CheckCircle" size={14} color={C.success}/>
                      <span style={{fontSize:11,fontWeight:800,color:C.success,letterSpacing:0.4}}>SON ÖDEMELER ({cari.son_odemeler?.length||0})</span>
                    </div>
                    {(!cari.son_odemeler || cari.son_odemeler.length===0) ? (
                      <div style={{padding:20,textAlign:'center',color:C.textMuted,fontSize:10}}>HENÜZ ÖDEME YOK</div>
                    ) : (
                      <table style={{width:'100%',fontSize:10,borderCollapse:'collapse'}}>
                        <thead><tr>
                          {['TARİH','TÜR','DÖNEM','TUTAR','KASA','AÇIKLAMA'].map(h => (
                            <th key={h} style={{padding:'6px 10px',textAlign:'left',fontSize:9,fontWeight:800,color:C.textSec,letterSpacing:0.5,borderBottom:`1px solid ${C.border}`}}>{h}</th>
                          ))}
                        </tr></thead>
                        <tbody>
                          {cari.son_odemeler.map(o => (
                            <tr key={o.id} style={{borderBottom:`1px solid ${C.border}33`}}>
                              <td style={{padding:'5px 10px',fontFamily:'monospace',color:C.text}}>{tarihFormat(o.odeme_tarihi)}</td>
                              <td style={{padding:'5px 10px'}}><span style={{padding:'1px 6px',borderRadius:3,fontSize:9,fontWeight:700,background:`${C.accent}22`,color:C.accent}}>{(o.odeme_turu||'').toUpperCase()}</span></td>
                              <td style={{padding:'5px 10px',fontFamily:'monospace',color:C.textSec}}>{o.donem || '—'}</td>
                              <td style={{padding:'5px 10px',fontFamily:'monospace',fontWeight:800,color:C.success}}>{paraBirim(o.tutar)}</td>
                              <td style={{padding:'5px 10px',color:C.textSec}}>{o.kasa_ad || '—'}</td>
                              <td style={{padding:'5px 10px',color:C.textMuted,fontSize:9}}>{o.aciklama || ''}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {/* ═══ SEKME 3: PRİM HAKEDİŞ (DÖNEM BAZLI ACCORDION) ═══ */}
              {detayTab === 'prim' && (
                <div>
                  <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12,padding:'10px 14px',background:C.bgCard,borderRadius:8,border:`1px solid ${C.border}`}}>
                    ...
                    <span style={{fontSize:11,fontWeight:700,color:C.textSec}}>
                      Her dönem KADEMELİ PRİM ile hesaplanır. Bir döneme tıklayınca o ay personelin
                      sorumlu olduğu DOSYALAR (ADK + BH) listelenir.
                    </span>
                  </div>
                  {cari.donemler.length === 0 ? (
                    <div style={{padding:40,textAlign:'center',color:C.textMuted,fontSize:11,background:C.bgCard,borderRadius:8,border:`1px dashed ${C.border}`}}>
                      ...
                      <div>HENÜZ HAKEDİŞ KAYDI BULUNAMADI</div>
                      <div style={{fontSize:9,marginTop:6}}>Bu personele sorumlu olarak dosya atandığında otomatik dolacak.</div>
                    </div>
                  ) : (
                    cari.donemler.map((d, idx) => {
                      const expanded = !!detayDonemAcik[idx];
                      const tumDosyalar = [...(d.adk_dosyalar||[]), ...(d.bh_dosyalar||[])];
                      return (
                        <div key={idx} style={{...S.card,marginBottom:10}}>
                          <div onClick={() => detayDonemToggle(idx)}
                            ...
                            <div style={{fontFamily:'monospace',fontWeight:800,color:C.text,fontSize:13,minWidth:80}}>{d.donem}</div>
                            <div style={{display:'flex',gap:6}}>
                              <span style={{padding:'2px 8px',borderRadius:4,fontSize:9,fontWeight:700,background:`${C.cyan}22`,color:C.cyan}}>ADK {d.adk_sayisi}</span>
                              <span style={{padding:'2px 8px',borderRadius:4,fontSize:9,fontWeight:700,background:`${C.purple}22`,color:C.purple}}>BH {d.bh_sayisi}</span>
                            </div>
                            <div style={{flex:1,fontSize:10,color:C.textSec,marginLeft:8}}>
                              MAAŞ {paraBirim(d.maas)} · ADK PRİM {paraBirim(d.adk_prim)} · BH PRİM {paraBirim(d.bh_prim)}
                            </div>
                            <div style={{fontFamily:'monospace',fontWeight:900,color:C.success,fontSize:13}}>{paraBirim(d.toplam_hakedis)}</div>
                            <span style={{padding:'2px 8px',borderRadius:4,fontSize:9,fontWeight:700,
                              background:(d.bakiye||0)<=0?`${C.success}22`:`${C.warning}22`,
                              color:(d.bakiye||0)<=0?C.success:C.warning}}>
                              {(d.bakiye||0)<=0 ? 'ÖDENDİ' : 'BAKİYE ' + paraBirim(d.bakiye)}
                            </span>
                          </div>
                          {expanded && (
                            <div style={{padding:'10px 14px',background:`${C.accent}05`}}>
                              {tumDosyalar.length === 0 ? (
                                <div style={{padding:14,textAlign:'center',color:C.textMuted,fontSize:10}}>BU DÖNEMDE DOSYA YOK (sadece maaş)</div>
                              ) : (
                                <table style={{width:'100%',fontSize:10,borderCollapse:'collapse'}}>
                                  <thead><tr>
                                    {['#','DOSYA NO','MAĞDUR','TÜR','AŞAMA','İL','AÇILIŞ','DURUM'].map(h2 => (
                                  ...
                                  <tbody>
                                    {tumDosyalar.map((f,fi) => (
                                      <tr key={f.dosya_id || fi} style={{borderBottom:`1px solid ${C.border}33`}}>
                                        <td style={{padding:'5px 8px',color:C.textMuted}}>{fi+1}</td>
                                        <td style={{padding:'5px 8px',fontFamily:'monospace',fontWeight:700,color:C.accent}}>{f.dosya_no}</td>
                                        <td style={{padding:'5px 8px',color:C.text}}>{f.magdur || '—'}</td>
                                        <td style={{padding:'5px 8px'}}><span style={{padding:'1px 6px',borderRadius:3,fontSize:9,fontWeight:700,background:f.dosya_turu==='BH'?`${C.purple}22`:`${C.cyan}22`,color:f.dosya_turu==='BH'?C.purple:C.cyan}}>{f.dosya_turu}</span></td>
                                        <td style={{padding:'5px 8px',color:C.textSec,fontSize:9}}>{f.asama || '—'}</td>
                                        <td style={{padding:'5px 8px',color:C.textMuted}}>{f.kaza_il || '—'}</td>
                                        <td style={{padding:'5px 8px',color:C.textMuted,fontFamily:'monospace',fontSize:9}}>{tarihFormat(f.acilis_tarihi)}</td>
                                        <td style={{padding:'5px 8px'}}>
                                          <span style={{padding:'1px 6px',borderRadius:3,fontSize:9,fontWeight:700,
                                            background:f.durum==='sayildi'?`${C.success}22`:`${C.warning}22`,
                                            color:f.durum==='sayildi'?C.success:C.warning}}>
                                            {(f.durum||'bekliyor').toUpperCase()}
                                          </span>
                                        </td>
                                      </tr>
                                    ))}
              ...
              {/* ═══ SEKME 4: ÇALIŞMA MODELİ — KADEME EDITOR ═══ */}
              {/* v2.24.1: detayModelForm artık detayAc başında her zaman set ediliyor */}
              {detayTab === 'model' && detayModelForm && typeof detayModelForm === 'object' && (
                <div>
                  {/* MODEL TİPİ + MAAŞ */}
                  <div style={{...S.card,marginBottom:12,padding:'14px 16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                      <LIcon name="Settings" size={14} color={C.accent}/>
                      <span style={{fontSize:11,fontWeight:800,color:C.accent,letterSpacing:0.4}}>KAZANÇ MODELİ TANIMI</span>
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:12}}>
                      <div>
                        <label style={{fontSize:9,fontWeight:700,color:C.textSec,letterSpacing:0.4}}>MODEL TİPİ</label>
                        <select value={detayModelForm.model_tipi}
                          onChange={e => setDetayModelForm({...detayModelForm, model_tipi:e.target.value})}
                          style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:4}}>
                          <option value="sabit">SADECE MAAŞ (sabit)</option>
                          <option value="sabit_prim">MAAŞ + SABİT PRİM/DOSYA</option>
                          <option value="kademeli_ofis">MAAŞ + KADEMELİ PRİM (ADK/BH AYRI) — OFİS</option>
                          <option value="kademeli_saha">MAAŞ + KADEMELİ PRİM (TOPLAM DOSYA) — SAHA</option>
                        </select>
                      </div>
                      <div>
                        <label style={{fontSize:9,fontWeight:700,color:C.textSec,letterSpacing:0.4}}>AYLIK SABİT MAAŞ (₺)</label>
                        <input type="number" value={detayModelForm.sabit_maas}
                          onChange={e => setDetayModelForm({...detayModelForm, sabit_maas:parseFloat(e.target.value)||0})}
                          style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:4,fontFamily:'monospace',fontWeight:800,color:C.success}}/>
                      </div>
                    </div>
                  </div>

                  {/* SABİT PRİM (ESKİ MODEL) */}
                  {detayModelForm.model_tipi === 'sabit_prim' && (
                    <div style={{...S.card,marginBottom:12,padding:'14px 16px'}}>
                      <div style={{fontSize:11,fontWeight:800,color:C.text,marginBottom:10}}>SABİT PRİM (dosya başına ₺)</div>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                        <div>
                          <label style={{fontSize:9,fontWeight:700,color:C.cyan,letterSpacing:0.4}}>ADK / MDK PRİM</label>
                          <input type="number" value={detayModelForm.prim_adk}
                            onChange={e => setDetayModelForm({...detayModelForm, prim_adk:parseFloat(e.target.value)||0})}
                            style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:4,fontFamily:'monospace',color:C.cyan}}/>
                        </div>
                        <div>
                          <label style={{fontSize:9,fontWeight:700,color:C.purple,letterSpacing:0.4}}>BH PRİM</label>
                          <input type="number" value={detayModelForm.prim_bh}
                            onChange={e => setDetayModelForm({...detayModelForm, prim_bh:parseFloat(e.target.value)||0})}
                            style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:4,fontFamily:'monospace',color:C.purple}}/>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* KADEMELİ OFİS (ADK + BH AYRI) */}
                  {detayModelForm.model_tipi === 'kademeli_ofis' && (
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                      <KademeEditor titre="ADK / MDK KADEMELERİ" renk={C.cyan}
                        kademeler={detayModelForm.ofis_adk_kademe}
                        onChange={k => setDetayModelForm({...detayModelForm, ofis_adk_kademe:k})}/>
                      <KademeEditor titre="BH KADEMELERİ" renk={C.purple}
                        kademeler={detayModelForm.ofis_bh_kademe}
                        onChange={k => setDetayModelForm({...detayModelForm, ofis_bh_kademe:k})}/>
                    </div>
                  )}

                  {/* KADEMELİ SAHA (TOPLAM DOSYA) */}
                  {detayModelForm.model_tipi === 'kademeli_saha' && (
                    <div style={{marginBottom:12}}>
                      <KademeEditor titre="TOPLAM DOSYA KADEMELERİ" renk={C.accent}
                        kademeler={detayModelForm.saha_kademe}
                        onChange={k => setDetayModelForm({...detayModelForm, saha_kademe:k})}/>
                    </div>
                  )}

                  {/* HESAPLAMA MANTIĞI BİLGİSİ */}
                  <div style={{padding:'12px 16px',background:`${C.accent}08`,borderRadius:8,border:`1px dashed ${C.accent}55`,marginBottom:12,fontSize:11,color:C.textSec,lineHeight:1.7}}>
                    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
                      <LIcon name="Info" size={12} color={C.accent}/>
                      <span style={{fontWeight:800,color:C.accent,fontSize:10,letterSpacing:0.4}}>PARÇALI (vergi gibi) HESAPLAMA</span>
                    </div>
                    Örnek 25 dosya · 3 kademe (0-10:400₺, 11-25:600₺, 26+:850₺) →<br/>
                    <b style={{color:C.text}}>İlk 10 × 400 = 4.000</b> + <b style={{color:C.text}}>Sonraki 15 × 600 = 9.000</b> = <b style={{color:C.success}}>13.000 ₺</b>
                  </div>

                  {/* KAYDET */}
                  <div style={{textAlign:'center'}}>
                    <button style={{...S.btn,...S.btnS,fontSize:12,padding:'10px 24px'}} onClick={modelKaydet}>
                      <LIcon name="Save" size={14} color="#fff"/> ÇALIŞMA MODELİNİ KAYDET
                    </button>
                  </div>
                </div>
              )}

              {/* ═══ SEKME 5: ÖZLÜK EVRAKLARI ═══ */}
              {detayTab === 'evrak' && (
                <div>
                  {/* YÜKLE FORMU */}
                  <div style={{...S.card,marginBottom:12,padding:'14px 16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                      <LIcon name="Upload" size={14} color={C.accent}/>
                      <span style={{fontSize:11,fontWeight:800,color:C.accent,letterSpacing:0.4}}>YENİ EVRAK YÜKLE (PDF / JPG / PNG / DOCX, max 10 MB)</span>
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1.5fr 2fr auto',gap:8,alignItems:'end'}}>
                      <div>
                        <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>EVRAK TİPİ</label>
                        <select value={evrakYukleForm.turu}
                          onChange={e => setEvrakYukleForm({...evrakYukleForm, turu:e.target.value})}
                          style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}>
                          {EVRAK_TIPLERI.map(t => <option key={t.k} value={t.k}>{t.l}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>ETİKET (opsiyonel)</label>
                        <input type="text" placeholder='Örn: "2026 Yıllık Sözleşme"'
                          value={evrakYukleForm.etiket}
                          onChange={e => setEvrakYukleForm({...evrakYukleForm, etiket:e.target.value})}
                          style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}/>
                      </div>
                      <div>
                        <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>DOSYA</label>
                        <input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.docx,.doc,.xlsx"
                          onChange={e => setEvrakYukleForm({...evrakYukleForm, file:e.target.files?.[0]||null})}
                          style={{...S.input,padding:'7px 10px',fontSize:10,width:'100%',marginTop:3}}/>
                      </div>
                      <button style={{...S.btn,...S.btnS,fontSize:10,padding:'9px 14px'}} onClick={evrakYukle}>
                        <LIcon name="Upload" size={11} color="#fff"/> YÜKLE
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={S.card}>
                    <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,background:`${C.accent}08`,display:'flex',alignItems:'center',gap:8}}>
                      ...
                      <span style={{fontSize:11,fontWeight:800,color:C.accent,letterSpacing:0.4}}>YÜKLÜ EVRAKLAR ({detayEvraklar.length})</span>
                    </div>
                    {detayEvraklar.length === 0 ? (
                      <div style={{padding:30,textAlign:'center',color:C.textMuted,fontSize:11}}>
                        <LIcon name="FileText" size={32} color={C.textMuted} style={{marginBottom:6}}/>
                        <div>HENÜZ EVRAK YÜKLENMEMİŞ</div>
                      </div>
                    ) : (
                      <table style={{width:'100%',fontSize:11,borderCollapse:'collapse'}}>
                        <thead style={{background:`${C.accent}10`}}>
                          <tr>
                            {['TİP','ETİKET','DOSYA ADI','BOYUT','YÜKLEYEN','TARİH','İŞLEM'].map(h => (
                              <th key={h} style={{padding:'8px 10px',textAlign:'left',fontSize:9,fontWeight:800,color:C.textSec,letterSpacing:0.5,borderBottom:`1px solid ${C.border}`}}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {detayEvraklar.map((e,i) => {
                            const tip = EVRAK_TIPLERI.find(t => t.k === e.evrak_turu);
                            return (
                              <tr key={e.id} style={{borderBottom:`1px solid ${C.border}66`,background: i%2===0?'transparent':`${C.accent}05`}}>
                                <td style={{padding:'7px 10px'}}>
                                  <span style={{padding:'2px 8px',borderRadius:4,fontSize:9,fontWeight:700,background:`${C.purple}22`,color:C.purple}}>{tip?.l || (e.evrak_turu||'').toUpperCase()}</span>
                                </td>
                                <td style={{padding:'7px 10px',color:C.text}}>{e.evrak_etiketi || '—'}</td>
                                <td style={{padding:'7px 10px',color:C.textSec,fontFamily:'monospace',fontSize:10}}>{e.dosya_adi}</td>
                                <td style={{padding:'7px 10px',color:C.textMuted,fontFamily:'monospace',fontSize:9}}>{Math.round((e.dosya_boyutu||0)/1024)} KB</td>
                                <td style={{padding:'7px 10px',color:C.textSec,fontSize:10}}>{e.yukleyen_ad || '—'}</td>
                                <td style={{padding:'7px 10px',color:C.textMuted,fontFamily:'monospace',fontSize:10}}>{tarihFormat(e.created_at)}</td>
                                <td style={{padding:'7px 10px'}}>
                                  <div style={{display:'flex',gap:6}}>
                                    <a href={`/api/v1/personel/evrak-indir.php?id=${e.id}`} target="_blank" rel="noopener noreferrer"
                                      style={{display:'inline-flex',alignItems:'center',gap:4,padding:'3px 8px',borderRadius:4,background:`${C.accent}22`,color:C.accent,fontSize:9,fontWeight:700,textDecoration:'none'}}>
                                      <LIcon name="Download" size={10} color={C.accent}/> İNDİR
                                    </a>
                                    <span onClick={() => evrakSil(e.id)}
                                      style={{cursor:'pointer',display:'inline-flex',alignItems:'center',gap:4,padding:'3px 8px',borderRadius:4,background:`${C.danger}22`,color:C.danger,fontSize:9,fontWeight:700}}>
                                      <LIcon name="Trash2" size={10} color={C.danger}/> SİL
                                    </span>
                                  </div>
                                </td>
                              </tr>
      ...
      {/* v2.24: CARİ HESAPTAN ÖDEME MODAL */}
      <Modal open={!!detayOdemeForm} onClose={() => setDetayOdemeForm(null)}
        title="PERSONELE ÖDEME YAP" width="500px">
        {detayOdemeForm && (
          <div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
              <div>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>ÖDEME TÜRÜ</label>
                <select value={detayOdemeForm.odeme_turu} onChange={e=>setDetayOdemeForm({...detayOdemeForm,odeme_turu:e.target.value})}
                  style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}>
                  <option value="maas">MAAŞ</option>
                  <option value="prim">PRİM</option>
                  <option value="avans">AVANS</option>
                  <option value="ikramiye">İKRAMİYE</option>
                  <option value="mesai">MESAİ</option>
                  <option value="diger">DİĞER</option>
                </select>
              </div>
              <div>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>DÖNEM (YYYY-MM)</label>
                <input type="month" value={detayOdemeForm.donem} onChange={e=>setDetayOdemeForm({...detayOdemeForm,donem:e.target.value})}
                  style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}/>
              </div>
              <div>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>TUTAR (₺)</label>
                <input type="number" value={detayOdemeForm.tutar} onChange={e=>setDetayOdemeForm({...detayOdemeForm,tutar:parseFloat(e.target.value)||0})}
                  style={{...S.input,padding:'8px 10px',fontSize:12,fontWeight:800,fontFamily:'monospace',width:'100%',marginTop:3,color:C.success}}/>
              </div>
              <div>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>KASA</label>
                <select value={detayOdemeForm.kasa_id} onChange={e=>setDetayOdemeForm({...detayOdemeForm,kasa_id:parseInt(e.target.value)})}
                  style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}>
                  {kasalar.map(k => <option key={k.id} value={k.id}>{k.ad} ({paraBirim(k.bakiye)})</option>)}
                </select>
              </div>
              <div>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>ÖDEME TARİHİ</label>
                <input type="date" value={detayOdemeForm.odeme_tarihi} onChange={e=>setDetayOdemeForm({...detayOdemeForm,odeme_tarihi:e.target.value})}
                  style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}/>
              </div>
              <div style={{gridColumn:'1 / -1'}}>
                <label style={{fontSize:9,fontWeight:700,color:C.textSec}}>AÇIKLAMA</label>
                <input type="text" value={detayOdemeForm.aciklama} onChange={e=>setDetayOdemeForm({...detayOdemeForm,aciklama:e.target.value})}
                  placeholder="Opsiyonel"
                  style={{...S.input,padding:'8px 10px',fontSize:11,width:'100%',marginTop:3}}/>
              </div>
            </div>
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:10}}>
              <button style={{...S.btn,...S.btnG,fontSize:11,padding:'8px 16px'}} onClick={() => setDetayOdemeForm(null)}>İPTAL</button>
              <button style={{...S.btn,...S.btnS,fontSize:11,padding:'8px 16px'}} onClick={detayOdemeKaydet}>
                <LIcon name="Wallet" size={12} color="#fff"/> ÖDEMEYİ KAYDET
              </button>
            </div>
          </div>
        )}
...
/* v2.24: KADEME EDITOR — parçalı kademe tablosu (min/max/birim/etiket) düzenleyici */
const KademeEditor = ({titre, renk, kademeler, onChange}) => {
  /* v2.24.2: LIcon erişimi (modül-top component, MR'dan çekilmeli — yoksa
     ReferenceError fırlatır ve ÇALIŞMA MODELİ sekmesi tamamen crash eder) */
  const C = MR.C, S = MR.S, LIcon = MR.LIcon;
  const list = Array.isArray(kademeler) ? kademeler : [];
  ...
  const addRow = () => {
    ...
    const nextMin = last ? (parseInt(last.max)||0) + 1 : 0;
    onChange([...list, {min: nextMin, max: nextMin+9, birim: 0, etiket:''}]);
  };
  const delRow = (i) => onChange(list.filter((_,idx) => idx !== i));
  return (
    <div style={{...S.card,padding:'12px 14px',border:`1px solid ${renk}33`,background:`${renk}06`}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
        <LIcon name="Layers" size={13} color={renk}/>
        <span style={{fontSize:11,fontWeight:800,color:renk,letterSpacing:0.4}}>{titre}</span>
        <button style={{...S.btn,...S.btnS,fontSize:9,padding:'4px 8px',marginLeft:'auto'}} onClick={addRow}>
          <LIcon name="Plus" size={10} color="#fff"/> KADEME EKLE
        </button>
      </div>
      {list.length === 0 ? (
        <div style={{padding:14,textAlign:'center',color:C.textMuted,fontSize:10,border:`1px dashed ${C.border}`,borderRadius:6}}>
          KADEME YOK — Bu modelde prim hesaplanmaz. KADEME EKLE ile başla.
        </div>
      ) : (
        <table style={{width:'100%',fontSize:10,borderCollapse:'collapse'}}>
          <thead><tr>
            {['#','MIN','MAX','BİRİM (₺)','ETİKET',''].map(h => (
              <th key={h} style={{padding:'5px 6px',textAlign:'left',fontSize:9,fontWeight:800,color:C.textSec,letterSpacing:0.4,borderBottom:`1px solid ${renk}33`}}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {list.map((k,i) => (
              <tr key={i}>
                <td style={{padding:'4px 6px',color:C.textMuted,fontFamily:'monospace'}}>{i+1}</td>
                <td style={{padding:'3px'}}><input type="number" value={k.min ?? 0}
                  onChange={e => setRow(i,'min',parseInt(e.target.value)||0)}
                  style={{...S.input,padding:'4px 6px',fontSize:10,width:60,fontFamily:'monospace'}}/></td>
                <td style={{padding:'3px'}}><input type="number" value={k.max ?? 0}
                  onChange={e => setRow(i,'max',parseInt(e.target.value)||0)}
                  style={{...S.input,padding:'4px 6px',fontSize:10,width:60,fontFamily:'monospace'}}/></td>
                <td style={{padding:'3px'}}><input type="number" value={k.birim ?? 0}
                  onChange={e => setRow(i,'birim',parseFloat(e.target.value)||0)}
                  style={{...S.input,padding:'4px 6px',fontSize:10,width:90,fontFamily:'monospace',fontWeight:700,color:renk}}/></td>
                <td style={{padding:'3px'}}><input type="text" value={k.etiket||''} placeholder="Kademe 1"
                  onChange={e => setRow(i,'etiket',e.target.value)}
                  style={{...S.input,padding:'4px 6px',fontSize:10,width:'100%'}}/></td>
                <td style={{padding:'3px'}}>
                  <span onClick={() => delRow(i)}
                    style={{cursor:'pointer',display:'inline-flex',padding:'3px 6px',borderRadius:3,background:`${C.danger}22`}}>
