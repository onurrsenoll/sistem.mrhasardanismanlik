/**
 * MR HASAR DANIŞMANLIK - POLİÇE TAKİP MODÜLÜ — v2.11.2
 * POLİÇE LİSTESİ, YENİ POLİÇE, YENİLEME TAKİBİ, TAHSİLAT/CARİ, RAPORLAR, KAZANÇ
 *
 * v2.11.2 düzeltmeleri (v2.11.1 üzerine):
 *  • YENİLEME TARİHİ boş kayıtlar artık "EKSİK BİLGİ" grubunda en üstte gösterilir
 *  • kalan_gun NULL-safe (NaN düşmüyor, kayıt görünmez kalmıyor)
 *  • Eksik tarih için "⚠ EKSİK" rozeti, kullanıcı DÜZENLE ile tamamlayabilir
 *  • Migration: bitis_tarihi/baslangic_tarihi/tanzim_tarihi NULL kabul eder
 *
 * v2.11.1 düzeltmeleri (v2.11 üzerine):
 *  • Yeni Excel şablonu — 10 kolon: YENİLEME TARİHİ | TELEFON | MÜŞTERİ ADI SOYADI |
 *    DOĞUM TARİHİ | VERGİ NO/T.C. NO | PLAKA | BELGE SERİ NO | ŞASE NUMARASI |
 *    MARKA | MODEL YILI
 *  • Eksik müşteri adı / sigorta şirketi → 'BELİRSİZ' placeholder, satır kaybolmaz
 *  • topluKaydet artık backend'in gerçek basarili/hatali sayısını gösterir
 *  • Daha sağlam tarih parser'ı (DD.MM.YYYY, DD/MM/YYYY, ISO, Excel serial, Date objesi)
 *  • Çakışmaya karşı police_no üretimi: timestamp + index + random
 *  • Yenileme tablosuna MARKA + MODEL YILI kolonları eklendi
 *
 * v2.11 düzeltmeleri:
 *  • PoliceYeni: bitiş tarihi useEffect'i kullanıcı manuel girdiyse override etmiyor
 *  • PoliceYenileme.topluKaydet: sigorta_sirketi mantık hatası giderildi
 *  • PoliceTahsilat: ölü kullanicilar state kaldırıldı
 *  • PoliceTahsilat.mForm initial state'inde devir_bakiye eklendi
...
/* v2.11.1: Yeni Excel şablonu — sıra ÖNEMLİ */
const YENILEME_EXCEL_HEADERS = [
  'YENİLEME TARİHİ',     // 0 → bitis_tarihi
  'TELEFON',              // 1 → musteri_telefon
  'MÜŞTERİ ADI SOYADI',   // 2 → musteri_adi
  'DOĞUM TARİHİ',         // 3 → dogum_tarihi
  'VERGİ NO / T.C. NO',   // 4 → musteri_tc
  'PLAKA',                // 5 → plaka
  'BELGE SERİ NO',        // 6 → belge_seri
  'ŞASE NUMARASI',        // 7 → sase_no
  'MARKA',                // 8 → marka
  'MODEL YILI'            // 9 → model_yili
];
...
/* v2.11.1: Daha sağlam tarih parser — Excel serial, JS Date, ISO, DD.MM.YYYY,
 * DD/MM/YYYY, DD-MM-YYYY hepsi ele alınır. Tanımadığında '' döner (NULL gönderim
 * için), eski sürümde ham string dönüp INSERT'i patlatıyordu. */
const parseExcelDate = (v) => {
  ...
  if (parts.length === 3) {
    let dd, mm, yyyy;
    if (parts[2].length === 4) { dd = parts[0]; mm = parts[1]; yyyy = parts[2]; }
    else if (parts[0].length === 4) { yyyy = parts[0]; mm = parts[1]; dd = parts[2]; }
    if (dd && mm && yyyy) {
...
const PoliceYeni = ({setPage, user, prefill}) => {
  ...
  const [basari, setBasari] = useState('');
  /* v2.11: Kullanıcı bitiş tarihini elle girdiyse otomatik +1yıl mantığı override etmesin */
  const [bitisManuel, setBitisManuel] = useState(false);
  const f = (k,v) => {
    if (k === 'bitis_tarihi') setBitisManuel(true);
    setForm(p => ({...p, [k]: v}));
  };

  // Başlangıç tarihi değişince bitiş tarihini +1 yıl yap (kullanıcı bitişi manuel girmediyse)
  useEffect(() => {
    if (form.baslangic_tarihi && !bitisManuel) {
      try {
        const dt = new Date(form.baslangic_tarihi);
        dt.setFullYear(dt.getFullYear() + 1);
        setForm(p => ({...p, bitis_tarihi: dt.toISOString().slice(0,10)}));
      } catch(e) {}
    }
  }, [form.baslangic_tarihi, bitisManuel]);
    ...
    <div style={S.card}>
      ...
      <div style={S.cardBody}>
        ...
        <div style={{display:'flex',gap:10}}>
          ...
          <button style={{...S.btn,...S.btnG,fontSize:13,padding:'12px 28px'}} onClick={()=>{setForm({...bosForm});setHata('');setBasari('');setBitisManuel(false);}}>
...
const PoliceYenileme = ({setPage, user, onPoliceyeDonustur}) => {
  ...
  const yukle = useCallback(async () => {
    ...
    if (r?.success) {
      ...
      /* v2.11.2: bitis_tarihi NULL/boş olan kayıtlar için kalan_gun = null
       * (NaN düşmesin, EKSİK BİLGİ grubuna alınsın) */
      const hesaplanmis = items.map(p => {
        if (!p.bitis_tarihi) return {...p, kalan_gun: null};
        const bitis = new Date(p.bitis_tarihi);
        if (isNaN(bitis.getTime())) return {...p, kalan_gun: null};
        bitis.setHours(0,0,0,0);
        const kalan = Math.ceil((bitis - bugun) / 86400000);
        return {...p, kalan_gun: kalan};
      }).sort((a,b) => {
        /* null kalan_gun en üstte (eksik bilgi en görünür) */
        if (a.kalan_gun === null && b.kalan_gun === null) return 0;
        if (a.kalan_gun === null) return -1;
        if (b.kalan_gun === null) return 1;
  ...
  // Gruplar — v2.11.2: EKSİK BİLGİ grubu eklendi (kullanıcı düzelt diye yükledi)
  const eksikBilgi = useMemo(() => policeler.filter(p => p.kalan_gun === null), [policeler]);
  const kritik = useMemo(() => policeler.filter(p => p.kalan_gun !== null && p.kalan_gun <= 15), [policeler]);
  ...
  /* v2.11.1: Excel ŞABLON İNDİR — yeni 10 kolonluk şablon
   * SIRA: YENİLEME TARİHİ | TELEFON | MÜŞTERİ ADI SOYADI | DOĞUM TARİHİ |
   *       VERGİ/T.C. NO | PLAKA | BELGE SERİ NO | ŞASE NUMARASI | MARKA | MODEL YILI */
  const downloadExcel = (policelerData) => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([YENILEME_EXCEL_HEADERS, ...rows]);
    ws['!cols'] = YENILEME_EXCEL_HEADERS.map(h => ({wch: Math.max(h.length+2, 16)}));
    XLSX.utils.book_append_sheet(wb, ws, 'YENİLEME LİSTESİ');
    XLSX.writeFile(wb, 'police_yenileme_sablon.xlsx');
  };

  /* v2.11.1: Excel YÜKLE — yeni şablon mapping'i, sağlam tarih parser,
   * çakışma-güvenli police_no üretimi, satır kaybetmeyecek mantık */
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        /* Header satırını atla; geri kalan satırlardan en az BİR alan dolu olanı al */
        const veriSatirlari = data.slice(1).filter(r =>
          Array.isArray(r) && r.some(c => c !== null && c !== undefined && String(c).trim() !== '')
        );
        if (veriSatirlari.length === 0) {
          setHata('EXCEL DOSYASINDA GEÇERLİ SATIR BULUNAMADI (BAŞLIK HARİÇ TÜM SATIRLAR BOŞ)');
          setTimeout(() => setHata(''), 5000);
          return;
        }
        /* Çakışma-güvenli police_no: timestamp + ri + 4 hane random */
        const tsBase = Date.now().toString();
        const rnd4 = () => Math.floor(Math.random() * 9000 + 1000).toString();
        const mapped = veriSatirlari.map((r, ri) => {
          const get = (idx) => {
            const v = r[idx];
          ...
          return {
            police_no: 'IMP-' + tsBase.slice(-8) + '-' + (ri + 1).toString().padStart(3,'0') + '-' + rnd4(),
            bitis_tarihi:    parseExcelDate(r[0]),
            musteri_telefon: get(1),
            musteri_adi:     get(2),
            dogum_tarihi:    parseExcelDate(r[3]),
            musteri_tc:      get(4),
            plaka:           get(5).toUpperCase().replace(/\s+/g,' '),
            belge_seri:      get(6),
            sase_no:         get(7).toUpperCase(),
            marka:           get(8).toUpperCase(),
            model_yili:      get(9),
            personel:        '',
            sigorta_sirketi: ''  /* topluKaydet'te BELİRSİZ ile doldurulur */
          };
        });
        setImportData(mapped);
        const eksikAd = mapped.filter(m => !m.musteri_adi).length;
        const eksikTarih = mapped.filter(m => !m.bitis_tarihi).length;
        let mesaj = mapped.length + ' SATIR OKUNDU.';
        if (eksikAd > 0)    mesaj += ' ' + eksikAd + " satırda MÜŞTERİ ADI eksik (BELİRSİZ olarak kaydedilecek).";
        if (eksikTarih > 0) mesaj += ' ' + eksikTarih + " satırda YENİLEME TARİHİ eksik (boş kaydedilecek).";
        mesaj += ' ÖNİZLEMEYİ KONTROL EDİP TOPLU KAYDET BUTONUNA BASIN.';
        setBasari(mesaj);
  ...
  /* v2.11.1: Toplu kaydet — backend'in gerçek basarili/hatali sayısını gösterir,
   * eksik müşteri adı / sigorta şirketi BELİRSİZ ile gönderilir, satır kaybolmaz */
  const topluKaydet = async () => {
    if (!importData || importData.length === 0) return;
    setLoading(true);
    const policelerPayload = importData.map(d => ({
      ...d,
      musteri_adi:     (d.musteri_adi && d.musteri_adi.trim())     || 'BELİRSİZ',
      sigorta_sirketi: (d.sigorta_sirketi && d.sigorta_sirketi.trim()) || 'BELİRSİZ',
    ...
    if (r?.success) {
      const basariliSayi = (r.data && (r.data.basarili ?? r.data.eklenen)) ?? importData.length;
      const hataliSayi   = (r.data && r.data.hatali) ?? 0;
      const toplam       = importData.length;
      const hatalar      = (r.data && r.data.hatalar) || [];

      setImportData(null);
      let mesaj = basariliSayi + ' / ' + toplam + ' POLİÇE BAŞARIYLA İÇE AKTARILDI';
      if (hataliSayi > 0) mesaj += ' (' + hataliSayi + ' SATIRDA HATA)';
      setBasari(mesaj);
      setTimeout(() => setBasari(''), 6000);

      /* Hata listesini kullanıcıya tek alert ile göster (ilk 10 satır) */
      if (hatalar.length > 0) {
        const ilk = hatalar.slice(0, 10).join('\n');
        setTimeout(() => alert('İÇE AKTARILAMAYAN SATIRLAR:\n\n' + ilk + (hatalar.length > 10 ? '\n... ve ' + (hatalar.length - 10) + ' satır daha' : '')), 100);
      }
      yukle();
    } else {
  ...
  // Tablo satırı render — v2.11.1: TELEFON 2. sütun, MARKA + MODEL YILI eklendi
  const renderRow = (p, i, renk) => (
    <tr key={p.id||i} style={{backgroundColor:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderBottom:`1px solid ${C.border}22`,borderLeft:`3px solid ${renk}66`,transition:'all .2s ease'}}>
      <td style={{...tdS,fontWeight:700,color:renk}}>{p.bitis_tarihi ? fmtTarih(p.bitis_tarihi) : '⚠ EKSİK'}</td>
      ...
      <td style={tdS}>{p.marka||'-'}</td>
      <td style={tdS}>{p.model_yili||'-'}</td>
      <td style={tdS}>
        <span style={{padding:'3px 8px',borderRadius:10,fontSize:10,fontWeight:800,background:`${renk}22`,color:renk,border:`1px solid ${renk}44`}}>
          {p.kalan_gun === null || p.kalan_gun === undefined ? '?' : p.kalan_gun + ' GN'}
        </span>
      </td>
      ...
      <div key={gi} style={{...S.card,marginBottom:16}}>
        ...
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:1180}}>
            <thead><tr style={{background:MR.tema==='koyu'?'#0f2342':'#1e40af'}}>
              {['YENİLEME','TELEFON','MÜŞTERİ','DOĞUM','TC/VERGİ','PLAKA','BELGE SERİ','ŞASE','MARKA','MODEL','KALAN','İŞLEM'].map(h=>
  ...
  return (
    <div>
      ...
      {/* İSTATİSTİK KARTLARI — v2.11.2: EKSİK BİLGİ kartı eklendi */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:12,marginBottom:20}}>
        <StatCard icon="HelpCircle" label="EKSİK BİLGİ" value={eksikBilgi.length} color={C.purple}/>
      ...
      {/* EXCEL İMPORT ÖNİZLEME — v2.11.1 yeni 10 kolon şablonu */}
        ...
        <div style={{...S.card,marginBottom:16}}>
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:10,minWidth:1100}}>
              <thead><tr style={{background:MR.tema==='koyu'?'#0f2342':'#1e40af'}}>
                {['YENİLEME','TELEFON','MÜŞTERİ','DOĞUM','TC/VERGİ','PLAKA','BELGE SERİ','ŞASE','MARKA','MODEL'].map(h=>
                  <th key={h} style={{padding:'8px 6px',textAlign:'left',color:'#fff',fontWeight:800,fontSize:'10px',borderBottom:`1px solid ${C.border}`}}>{h}</th>
                )}
              </tr></thead>
              <tbody>
                {importData.slice(0,50).map((d,i)=>{
                  const eksik = !d.musteri_adi || !d.bitis_tarihi;
                  return (
                  <tr key={i} style={{backgroundColor:MR.tema==='koyu'?(i%2===0?'#111827':'#0d1321'):(i%2===0?'#ffffff':'#f0f4ff'),borderBottom:`1px solid ${C.border}22`,borderLeft: eksik ? `3px solid ${C.warning}` : `3px solid ${C.accent}55`}}>
                    <td style={{padding:'6px',fontSize:'11px',fontWeight:600,color:!d.bitis_tarihi?C.warning:(MR.tema==='koyu'?'#e2e8f0':'#1e293b')}}>{d.bitis_tarihi ? fmtTarih(d.bitis_tarihi) : '⚠ EKSİK'}</td>
                    <td style={{padding:'6px',fontSize:'11px',fontWeight:600,color:MR.tema==='koyu'?'#e2e8f0':'#1e293b'}}>{d.musteri_telefon||'-'}</td>
                    <td style={{padding:'6px',fontWeight:700,fontSize:'11px',color:!d.musteri_adi?C.warning:(MR.tema==='koyu'?'#e2e8f0':'#1e293b')}}>{d.musteri_adi || '⚠ BELİRSİZ'}</td>
                    ...
                    <td style={{padding:'6px',fontSize:'11px',fontWeight:600,color:MR.tema==='koyu'?'#e2e8f0':'#1e293b'}}>{d.marka||'-'}</td>
                    <td style={{padding:'6px',fontSize:'11px',fontWeight:600,color:MR.tema==='koyu'?'#e2e8f0':'#1e293b'}}>{d.model_yili||'-'}</td>
                  </tr>
                );})}
              </tbody>
            </table>
      ...
      {policeler.length === 0 ? (
        ...
        <>
          {renderGrup('EKSİK BİLGİ', 'YENİLEME TARİHİ EKSİK — DÜZENLE İLE TAMAMLAYIN', eksikBilgi, C.purple, 'HelpCircle', -1)}
          ...
          <div style={{width:500,maxHeight:'80vh',overflowY:'auto',background:C.bgCard||'#fff',borderRadius:12,padding:0,boxShadow:'0 20px 60px rgba(0,0,0,0.4)'}}>
            ...
            <div style={{padding:20,display:'grid',gap:12}}>
              {[
                ...
                {key:'model_yili',label:'MODEL YILI'}
              ].map(f => (
                <div key={f.key}>
...
const PoliceTahsilat = ({setPage, user}) => {
  ...
  const [policeler, setPoliceler] = useState([]);
  const [kasalar, setKasalar] = useState([]);
  /* v2.11: ölü kullanicilar state'i kaldırıldı, gereksiz API çağrısı yok */
  ...
  /* v2.11: mForm initial state'inde devir_bakiye eksikti, undefined input warning veriyordu */
  const [mForm, setMForm] = useState({devir_tarih:'',devir_bakiye:'',aciklama:'',donem:''});
...
const PoliceKazanc = ({setPage, user}) => {
  ...
  const [basari, setBasari] = useState('');
  /* v2.11: ölü donem state'i kaldırıldı (hiç okunmuyordu) */
