 ...
 *   Silinmiş dosyaların masrafları artık listede görünmez (zombie kayıt yok).
 *   Bağlı dosyası olmayan masraflar (genel kasa hareketleri) korunur.
 */
...
/* v2.7 (#034): Soft-delete cascade —
   masraflar.dosya_id ya null (genel masraf) ya da bağlı dosya silinmemiş olmalı */
$where[] = '(m.dosya_id IS NULL OR d.silindi = 0 OR d.silindi IS NULL)';

$whereSQL = !empty($where) ? 'WHERE ' . implode(' AND ', $where) : '';

// COUNT için JOIN gerekli (silindi filtresi için)
$stmt = $db->prepare("SELECT COUNT(*) as total FROM masraflar m LEFT JOIN dosyalar d ON d.id = m.dosya_id $whereSQL");
$stmt->execute($params);
