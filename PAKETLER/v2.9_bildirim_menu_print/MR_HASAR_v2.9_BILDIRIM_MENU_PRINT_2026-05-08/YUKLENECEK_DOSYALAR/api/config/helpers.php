...
/* ════════════════════════════════════════════════════════════════
   v2.1.2 BİLDİRİM HELPER'LARI
   Tüm modüller bu fonksiyonları kullanmalı (şema tutarlılığı için).
   bildirimler tablosu standart şeması:
     alici_id, gonderen_id, baslik, icerik, tip, ilgili_dosya_id, okundu, created_at
   ════════════════════════════════════════════════════════════════ */

/**
 * Sistem-içi bildirim oluşturur.
 * Hata olursa sessizce loga yazar — çağıran ana akış kesilmez.
 *
 * @param int      $alici_id        Bildirim alacak users.id
 * @param string   $baslik          Kısa başlık
 * @param string   $icerik          Detaylı mesaj
 * @param string   $tip             bilgi|uyari|basari|acil|sistem|bildirim|mesaj (default: bilgi)
 * @param int|null $ilgili_dosya_id Varsa dosya referansı (link tıklama için)
 * @param int|null $gonderen_id     Varsa gönderen users.id (otomatik tetiklemelerde null)
 * @return int Yeni bildirim ID'si veya hata durumunda 0
 */
function bildirim_olustur($alici_id, $baslik, $icerik = '', $tip = 'bilgi', $ilgili_dosya_id = null, $gonderen_id = null) {
    if (empty($alici_id) || (int)$alici_id <= 0) return 0;
    $gecerliTipler = array('bilgi', 'uyari', 'basari', 'acil', 'sistem', 'bildirim', 'mesaj', 'hata');
    if (!in_array($tip, $gecerliTipler, true)) $tip = 'bilgi';
    try {
        $db = getDB();
        $stmt = $db->prepare('INSERT INTO bildirimler (alici_id, gonderen_id, baslik, icerik, tip, ilgili_dosya_id) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute(array(
            (int)$alici_id,
            !empty($gonderen_id) ? (int)$gonderen_id : null,
            mb_substr((string)$baslik, 0, 200),
            (string)$icerik,
            $tip,
            !empty($ilgili_dosya_id) ? (int)$ilgili_dosya_id : null
        ));
        return (int)$db->lastInsertId();
    } catch (Exception $e) {
        error_log('[mr_hasar] bildirim_olustur: ' . $e->getMessage());
        return 0;
    }
}

/**
 * Bir 'ortaklar.id' verildiğinde, e-posta üzerinden bağlantılı users.id'yi döndürür.
 * Kullanım: $userId = ortak_user_id_bul($ortakId); if ($userId) bildirim_olustur(...);
 *
 * @param int $ortak_id ortaklar.id
 * @return int|null users.id veya null (eşleşme yoksa)
 */
function ortak_user_id_bul($ortak_id) {
    if (empty($ortak_id)) return null;
    try {
        $db = getDB();
        $stmt = $db->prepare("SELECT u.id FROM users u INNER JOIN ortaklar o ON LOWER(u.email) = LOWER(o.email) WHERE o.id = ? AND u.aktif = 1 AND o.email <> '' LIMIT 1");
        $stmt->execute(array((int)$ortak_id));
        $row = $stmt->fetch();
        return $row ? (int)$row['id'] : null;
    } catch (Exception $e) {
        error_log('[mr_hasar] ortak_user_id_bul: ' . $e->getMessage());
        return null;
    }
}

/**
 * Bir users.id verildiğinde aktif & geçerli mi kontrol eder.
 * Bildirim göndermeden önce hızlı doğrulama için.
 */
function bildirim_alici_gecerli($user_id) {
    if (empty($user_id)) return false;
    try {
        $db = getDB();
        $stmt = $db->prepare('SELECT 1 FROM users WHERE id = ? AND aktif = 1 LIMIT 1');
