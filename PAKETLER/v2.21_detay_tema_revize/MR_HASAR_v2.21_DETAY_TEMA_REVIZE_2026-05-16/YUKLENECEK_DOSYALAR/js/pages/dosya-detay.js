/**
 * MR HASAR DANIŞMANLIK — DOSYA DETAY (v2.21)
...
MR._DosyaDetayInfoRow = ({label, value, mono, bold, color}) => {
  ...
  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:'1px solid ' + C.border}}>
      ...
      <span style={{fontSize:11.5,fontWeight: bold ? 800 : 700, color: color || C.text, fontFamily: mono ? 'monospace' : 'inherit',textAlign:'right',maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{value || '-'}</span>
    </div>
  );
};
...
/* v2.21: Araç bilgi banner'ı — ikon/logo kaldırıldı (amatör görünüm).
 * Tek banner: SOL marka/model/yıl tipografisi, SAĞ araç sahibi/ruhsat sahibi.
 * Logo opsiyonel olarak sağ üst köşede minik gösterilir (yüklenmezse gizli). */
const AracGorsel = ({arac, anaRenk}) => {
  ...
  return (
    <div style={{
      background: `linear-gradient(135deg, ${anaRenk}10 0%, ${anaRenk}18 50%, ${anaRenk}0a 100%)`,
      borderRadius:10,
      padding:'14px 16px',
      position:'relative', overflow:'hidden',
      border:`1px solid ${anaRenk}33`,
      boxShadow:`inset 0 1px 0 rgba(255,255,255,0.06), 0 1px 4px ${anaRenk}10`
    }}>
      {/* Opsiyonel minik logo — sağ üst köşe (yüklenmezse hiç görünmez) */}
      {logoUrl && !logoErr && (
        <img src={logoUrl} alt="" onError={() => setLogoErr(true)}
          style={{position:'absolute',top:10,right:10,width:36,height:36,objectFit:'contain',opacity:0.85}}/>
      )}
      <div style={{display:'grid', gridTemplateColumns: sahipVar ? '1.15fr 1px 1fr' : '1fr', gap:14, alignItems:'stretch'}}>
        {/* SOL: ARAÇ */}
        <div>
          <div style={{fontSize:9,fontWeight:800,color:anaRenk,letterSpacing:0.8,marginBottom:4,opacity:0.85}}>ARAÇ</div>
          <div style={{fontSize:20,fontWeight:900,color:anaRenk,letterSpacing:0.4,textTransform:'uppercase',lineHeight:1.15}}>
            {arac.marka || '—'}
          </div>
          <div style={{fontSize:13,fontWeight:800,color:C.text,marginTop:3,textTransform:'uppercase',lineHeight:1.2,letterSpacing:0.2}}>
            {arac.model || '—'}
          </div>
          <div style={{fontSize:11,color:C.textSec,marginTop:4,fontWeight:700,fontFamily:'monospace'}}>
        ...
        {/* SAĞ: ARAÇ SAHİBİ / RUHSAT SAHİBİ */}
        {sahipVar && (
          <div>
            <div style={{fontSize:9,fontWeight:800,color:anaRenk,letterSpacing:0.8,marginBottom:4,opacity:0.85}}>ARAÇ SAHİBİ / RUHSAT SAHİBİ</div>
            <div style={{fontSize:13,fontWeight:800,color:C.text,textTransform:'uppercase',lineHeight:1.2,letterSpacing:0.2,marginTop:2}}>
              {arac.ruhsat_sahibi || '—'}
            </div>
            {arac.tc_kimlik && (
        ...
        <div style={{padding:'16px 22px 12px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:14}}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            ...
            <select value={dosya.asama} onChange={e => asamaDegistirOnay(e.target.value)}
              style={{...S.select,minWidth:200,fontSize:10,padding:'7px 10px',background:`${ac}11`,border:`1px solid ${ac}33`,color:ac,fontWeight:700}}>
